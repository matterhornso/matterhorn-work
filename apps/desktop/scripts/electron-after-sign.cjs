...
const computerUseHelperAppName = "Matterhorn Desks Automation Helper.app";
...
function verifyComputerUseHelper(appPath, requireDistributionSignature) {
  ...
  if (!existsSync(helperPath)) {
    throw new Error(`Matterhorn Desks automation helper app is missing from packaged app: ${helperPath}`);
  }
  ...
  if (result.status !== 0) {
    throw new Error(`codesign --display failed for Matterhorn Desks automation helper with status ${result.status}`);
  }
  if (result.stderr.includes("Signature=adhoc")) {
    throw new Error("Matterhorn Desks automation helper app is ad-hoc signed; notarized builds require a Developer ID signature.");
  }
}
