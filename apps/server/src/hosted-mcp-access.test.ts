...
describe("hosted MCP access credentials", () => {
  test("stores only a hash and resolves one account-bound expiring credential", () => {
    ...
    const row = db.query(
      "SELECT token_hash, label FROM hosted_mcp_access_tokens WHERE id = ?",
    ).get(issued.id) as { token_hash: string; label: string };
    db.close();
  ...
  test("caps active credentials and removes only metadata older than 365 days", () => {
    ...
    db.query(
      "UPDATE hosted_mcp_access_tokens SET revoked_at = ? WHERE id = ?",
    ).run(now - 366 * DAY_MS, credentials[0]!.id);
    db.query(
      "UPDATE hosted_mcp_access_tokens SET revoked_at = ? WHERE id = ?",
    ).run(now - DAY_MS, credentials[1]!.id);
    db.close();

    expect(store.maintainEphemeralSecurityState(now).expiredHostedMcpCredentialsDeleted)
      .toBe(1);
