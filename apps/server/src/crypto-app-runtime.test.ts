...
describe("crypto app runtime startup", () => {
  test("performs no database access while the gateway is off", () => {
    ...
    expect(runtime.maintainConnectionSetupMetadata()).toEqual({
  ...
  test("requires an explicit policy version and trusted public keyring", () => {
    ...
    expect(() => createMatterhornCryptoAppRuntime(missingOperationalIntegritySecret))
      .toThrowError(expect.objectContaining({ code: "crypto_app_operational_integrity_secret_required" }));
