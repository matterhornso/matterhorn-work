...
describe("Shared primitives UI contract", () => {
  ...
  test("settings shell avoids oversized radius on error surfaces", () => {
    ...
    expect(source).not.toContain("rounded-2xl");
    expect(source).toContain('className="flex h-9 shrink-0 items-center justify-between px-4 md:hidden mac:titlebar-drag"');
    expect(source).not.toContain("<ChevronRight");
    expect(source).not.toContain('h-10 items-center justify-between border-b border-dls-border');
  });
  ...
  test("reload toast uses the same opaque notification surface", () => {
    const source = readAppSource("domains/shell-feedback/reload-workspace-toast.tsx");
  ...
  test("session composer avoids oversized pill controls", () => {
    ...
    expect(composerSource).toContain("focus-within:bg-dls-surface-muted/[0.23]");
  ...
  test("composer tool menu uses compact tabs and a calm loading state", () => {
    const source = readAppSource("domains/session/surface/composer/composer.tsx");
  ...
  test("settings and session navigation expose current-page semantics", () => {
    ...
    expect(settingsTabs).toContain('aria-current={props.active ? "page" : undefined}');
    expect(appSidebar).toContain('aria-current={ctx.selectedWorkspaceId === workspace.id ? "page" : undefined}');
    expect(appSidebar.match(/aria-current={isSelected \? "page" : undefined}/g)?.length ?? 0).toBeGreaterThanOrEqual(2);
  });
  ...
  test("settings sidebar uses quiet navigation instead of outlined status pills", () => {
    const settingsPage = readAppSource("domains/settings/shell/settings-page.tsx");

    expect(settingsPage).toContain("SETTINGS_SIDEBAR_ITEM_CLASS");
    expect(settingsPage).toContain('case "overview":\n      return LayoutDashboard;');
    expect(settingsPage).toContain('case "ai":\n      return Cpu;');
    expect(settingsPage).not.toContain('case "ai":\n      return Zap;');
    expect(settingsPage).toContain("data-active:bg-transparent");
    expect(settingsPage).toContain("data-active:[&_svg]:text-[rgb(var(--matterhorn-blue-rgb))]");
    ...
    expect(settingsPage).not.toContain("border-sky-500/30 bg-sky-500/10 text-sky-300");
    expect(settingsPage).not.toContain("rounded-md border px-1.5 py-0.5 text-[9px]");
  });
  ...
  test("Customization uses open sections and inline guidance instead of framed boxes", () => {
    ...
    expect(shellView).toContain('<div className="ml-6 flex flex-col gap-3 pl-4">');
    expect(shellView).not.toContain('from "@/components/ui/alert"');
    expect(shellView).not.toContain("<Separator />");
    expect(shellView).not.toContain('className="rounded-lg border border-dls-border p-4"');
    expect(shellView).not.toContain('variant="outline"\n          size="sm"\n          onClick={resetAll}');
    expect(shellView).toContain("<LayoutSectionItemTitle>Application name</LayoutSectionItemTitle>");
    ...
    expect(shellView).not.toContain('unavailable="The model picker display control is not available yet."');
    expect(shellView).not.toContain('unavailable="The new workspace button display control is not available yet."');
    expect(shellView).not.toContain("cloudOnly");

    const composer = readAppSource("domains/session/surface/composer/composer.tsx");
    const sessionSurface = readAppSource("domains/session/surface/session-surface.tsx");
    const sidebar = readAppSource("domains/session/sidebar/app-sidebar.tsx");
    expect(composer).toContain("props.showModelPicker !== false");
    expect(sessionSurface).toContain("showModelPicker={shellConfig.modelPicker}");
  ...
  test("billing management uses a quiet action instead of an outlined control", () => {
    const billingView = readAppSource("domains/settings/pages/billing-view.tsx");
    ...
    expect(billingView).toContain("bg-dls-surface-muted/[0.13] text-dls-secondary");
    expect(billingView).toContain("border-0 bg-dls-surface-muted/[0.13] px-2 text-xs text-dls-secondary");
    expect(billingView).toContain("hover:bg-dls-surface-muted/[0.2]");
    expect(billingView).toContain("Active for this workspace");
    expect(billingView).not.toContain("rounded-md border-dls-border/35 text-xs shadow-none");
  });
  ...
  test("main Matterhorn surfaces avoid harsh divider-line scaffolding", () => {
    const sourceByPath = new Map([
      ...
      ["domains/settings/backend-capabilities/backend-capability-status.tsx", readAppSource("domains/settings/backend-capabilities/backend-capability-status.tsx")],
      ["domains/profile/profile-capability-status.tsx", readAppSource("domains/profile/profile-capability-status.tsx")],
      ...
      ["domains/settings/pages/generated-media-view.tsx", readAppSource("domains/settings/pages/generated-media-view.tsx")],
      ["domains/settings/pages/marketplace-view.tsx", readAppSource("domains/settings/pages/marketplace-view.tsx")],
      ...
      ["domains/session/sidebar/app-sidebar.tsx", readAppSource("domains/session/sidebar/app-sidebar.tsx")],
      ["domains/session/surface/message-list.tsx", readAppSource("domains/session/surface/message-list.tsx")],
      ["domains/session/surface/tool-call.tsx", readAppSource("domains/session/surface/tool-call.tsx")],
      ["domains/wallet/pages/BittensorPanel.tsx", readAppSource("domains/wallet/pages/BittensorPanel.tsx")],
    ...
    expect(sourceByPath.get("domains/session/media/session-image-generation-panel.tsx")).toContain("bg-dls-surface-muted/[0.035]");
    expect(sourceByPath.get("domains/session/workflows/workflow-stage-card.tsx")).toContain("bg-dls-surface-muted/[0.20]");
    expect(sourceByPath.get("domains/session/workflows/workflow-stage-card.tsx")).toContain("bg-dls-surface-muted/[0.38]");
    expect(sourceByPath.get("domains/session/surface/message-list.tsx")).toContain("function MessageActionIconButton");
    expect(sourceByPath.get("domains/session/surface/message-list.tsx")).toContain('block.isUser ? "absolute right-2 top-2" : "mt-2"');
    expect(sourceByPath.get("domains/session/surface/message-list.tsx")).toContain("bg-dls-surface-muted/[0.14] ring-1 ring-white/[0.08]");
    expect(sourceByPath.get("domains/session/surface/message-list.tsx")).not.toContain("absolute bottom-2 flex items-center");
    expect(sourceByPath.get("domains/session/surface/message-list.tsx")).not.toContain("border border-dls-border bg-dls-sidebar");
    expect(sourceByPath.get("domains/wallet/sui-workflow-panel.tsx")).toContain("SUI_PANEL_SECTION_CLASS");
    expect(sourceByPath.get("domains/wallet/sui-workflow-panel.tsx")).toContain("matterhorn-rail-section grid gap-3 py-2");
    expect(sourceByPath.get("domains/wallet/sui-workflow-panel.tsx")).not.toContain('SUI_PANEL_SECTION_CLASS = "grid gap-3 rounded-md');
    expect(sourceByPath.get("domains/settings/pages/wallet-view.tsx")).toContain("[grid-template-columns:repeat(auto-fit,minmax(min(100%,8.5rem),1fr))]");
    expect(sourceByPath.get("domains/settings/pages/wallet-view.tsx")).toContain("Connected wallet");
    expect(sourceByPath.get("domains/settings/pages/wallet-view.tsx")).toContain("bg-dls-surface-muted/[0.055]");
    expect(sourceByPath.get("domains/settings/pages/wallet-view.tsx")).not.toContain("@/components/ui/card");
    expect(sourceByPath.get("domains/settings/pages/wallet-view.tsx")).not.toContain("@/components/ui/separator");
    expect(sourceByPath.get("domains/settings/pages/wallet-view.tsx")).not.toContain("<Card");
    expect(sourceByPath.get("domains/settings/pages/wallet-view.tsx")).not.toContain("<Separator");
    expect(sourceByPath.get("domains/settings/pages/wallet-view.tsx")).not.toContain("divide-y");
    expect(sourceByPath.get("domains/settings/pages/wallet-view.tsx")).not.toContain("border border-dls-border");
    expect(sourceByPath.get("domains/settings/pages/wallet-view.tsx")).not.toContain("border-dls-border/30");
    expect(sourceByPath.get("domains/settings/pages/wallet-view.tsx")).not.toContain("border-b border-dls-border/45");
    expect(sourceByPath.get("domains/settings/pages/cloud-account-view.tsx")).not.toContain("@/components/ui/separator");
    expect(sourceByPath.get("domains/settings/pages/cloud-account-view.tsx")).not.toContain("<Separator");
    expect(sourceByPath.get("domains/settings/pages/cloud-account-view.tsx")).toContain("<DenSignedOutPanel");
    expect(sourceByPath.get("domains/settings/pages/cloud-account-view.tsx")).toContain("compact");
    expect(sourceByPath.get("domains/wallet/pages/BittensorPanel.tsx")).not.toContain("aria-label={`Step");
    expect(sourceByPath.get("domains/settings/pages/generated-media-view.tsx")).toContain("grid gap-1");
    expect(sourceByPath.get("domains/settings/pages/marketplace-view.tsx")).toContain("bg-dls-surface-muted/[0.055]");
    expect(sourceByPath.get("domains/settings/pages/marketplace-view.tsx")).not.toContain("border-b border-gray");
    expect(sourceByPath.get("domains/settings/pages/marketplace-view.tsx")).not.toContain("divide-y divide-gray");
    expect(sourceByPath.get("domains/settings/pages/marketplace-view.tsx")).not.toContain("tracking-wider");
    expect(sourceByPath.get("domains/settings/pages/marketplace-view.tsx")).not.toContain("h-[2px]");
    expect(sourceByPath.get("domains/profile/profile-capability-status.tsx")).toContain("grid min-w-0 gap-1 pl-0 sm:pl-12");
    expect(sourceByPath.get("domains/settings/backend-capabilities/backend-capability-status.tsx")).toContain("inline-flex min-w-0 items-center gap-1.5 text-xs font-medium");
    expect(sourceByPath.get("domains/settings/backend-capabilities/backend-capability-status.tsx")).not.toContain("rounded-md border px-2 py-0.5");
  });
  ...
  test("compact MCP settings use selection and disclosure instead of a long card document", () => {
    ...
    expect(source).toContain('"rounded-md bg-transparent transition-colors hover:bg-dls-surface-muted/[0.08]"');
    expect(source).toContain('className="matterhorn-mcp-stream grid min-w-0 gap-1"');
    expect(source).toContain('? "grid gap-1"');
    expect(source).toContain('cn("rounded-md bg-transparent transition-colors"');
    expect(source).toContain('? "px-1 py-2"');
    expect(source).not.toContain('className="matterhorn-mcp-stream grid min-w-0 gap-0.5 rounded-lg bg-dls-surface-muted/[0.055] p-1"');
    ...
    expect(source).toContain("onCopyCommand={copyMatterhornMcpCommand}\n        compact");
    expect(extensionsSource).toContain("mcpConnectedAppNames: string[]");
    expect(extensionsSource).toContain('connectedAppNames.join(" · ")');
    expect(extensionsSource).toContain("h-9 min-w-0 rounded-none border-x-0 border-t-0 border-b-2");
    expect(extensionsSource).toContain('"grid min-w-0 flex-1 grid-cols-2 border-b border-dls-border/40"');
    expect(extensionsSource).toContain("const marketplaceAvailable = Boolean(props.cloudMarketplaceView)");
    expect(extensionsSource).toContain("!marketplaceAvailable ? refreshButton : null");
    expect(extensionsSource).toContain(") : props.cloudMarketplaceView}");
    expect(extensionsSource).not.toContain("Marketplace extensions are post-go-live");
    expect(extensionsSource).toContain("{refreshButton}");
    expect(settingsRoute).toContain("cloudMarketplaceView={MATTERHORN_CLOUD_ENABLED ? (");
    expect(source).toContain('className="text-xs font-medium text-dls-secondary"');
    expect(settingsRoute).toContain('mcpStatuses[server.name]?.status === "connected"');
    expect(settingsRoute).not.toContain("connectionsSnapshot.mcpServers.length");
  ...
  test("web workspaces refresh MCP truth through the connected backend", () => {
    ...
    expect(connectionsStore).not.toContain("!started || disposed || !isDesktopRuntime() || !changed");
    expect(settingsRoute).toContain('matterhornServerSnapshot.matterhornServerStatus !== "connected" || !runtimeWorkspaceId');
    expect(settingsRoute).toContain("matterhornServerSnapshot.matterhornServerStatus, runtimeWorkspaceId");
  });
  ...
  test("right-side panels use the workspace canvas and open compact sections", () => {
    ...
    expect(sessionPage).toContain('const embeddedSettingsPanelOpen = visibleSidePanel === "extensions" || visibleSidePanel === "profile" || visibleSidePanel === "wallet";');
    expect(sessionPage).toContain("{!embeddedSettingsPanelOpen ? (");
    expect(settingsShell).toContain("shadow-[0_1px_0_rgba(var(--matterhorn-blue-rgb),0.08)]");
    ...
    expect(walletView).toContain("rounded-lg bg-dls-surface-muted/[0.14] p-1.5");
    expect(walletView).toContain("border-0 bg-dls-surface-muted/[0.10] px-3 py-2.5");
  });
  ...
  test("tablet side panels stay visible until the docked lg layout is available", () => {
    ...
    expect(sessionPage).toContain('className="matterhorn-side-panel hidden h-full min-h-0 overflow-hidden bg-dls-background lg:flex lg:flex-col"');
    expect(sessionPage).toContain('className="matterhorn-side-panel fixed inset-0 z-40 flex bg-dls-background lg:hidden"');
  });
  ...
  test("Profile rail opens the account surface instead of the Settings index", () => {
    const sessionPage = readAppSource("domains/session/chat/session-page.tsx");
    const accountView = readAppSource("domains/settings/pages/cloud-account-view.tsx");
    ...
    expect(sessionPage).toContain('params.set("panel", panel)');
    expect(sessionPage).toContain('params.delete("panel")');
    expect(readAppSource("domains/settings/shell/settings-shell.tsx")).toContain('props.compact && props.activeTab === "cloud-account"');
