...
describe("AI provider UI contract", () => {
  test("does not present an empty catalog while connected models are still loading", () => {
    const pickerSource = readReactSource("domains/session/modals/model-picker-modal.tsx");
    const routeSource = readReactSource("shell/settings-route.tsx");

    expect(pickerSource).toContain("Loading available models...");
    expect(pickerSource).toContain("props.loading && providerGroups.length === 0");
  ...
  test("uses backend catalog truth while the direct provider store catches up", () => {
    ...
    expect(viewSource).not.toContain("catalog?.serverFetched ? catalog.modelCount");
  });

  test("keeps technical model details scoped to connected providers", () => {
    const summarySource = readReactSource("domains/settings/state/model-readiness-summary.ts");

    expect(summarySource).toContain("options: { connectedOnly?: boolean } = {}");
    expect(summarySource).toContain("buildModelCatalogRows(catalog, { connectedOnly: true })");
  ...
  test("does not advertise Matterhorn Cloud model subscriptions in a local-only build", () => {
    ...
    expect(routeSource).toContain("showOpenWorkModelsSubscribe={showOpenWorkModelsSubscribe}");
  });
});
