...
import type { MatterhornNftReceiptRequest } from "@matterhorn-work/types/generated-media";
import { suiDAppKit } from "../../../infra/sui-dapp-kit";
import {
  NftDraftPanel,
  type NftDraftPanelProps,
  type NftWalletExecutionState,
...
export type NftDraftWalletBridgeProps = Omit<
  NftDraftPanelProps,
  "walletExecution"
>;
...
function NftDraftWalletBridgeContent(props: NftDraftWalletBridgeProps) {
  ...
  const [lastMintReceipt, setLastMintReceipt] =
    useState<MatterhornSuiWalletExecutionReceipt | null>(null);
  const [lastListingReceipt, setLastListingReceipt] =
    useState<MatterhornSuiWalletExecutionReceipt | null>(null);
  ...
  const connectWallet = useCallback(
    async (walletId: string) => {
      const nextWallet = wallets.find(
        (availableWallet) =>
          walletOptionId(availableWallet.name, availableWallet.version) ===
          walletId,
      );
      if (!nextWallet) {
  ...
  const disconnectWallet = useCallback(async () => {
    ...
    try {
      ...
      setWalletError(
        error instanceof Error
          ? error.message
          : "Could not disconnect Sui wallet.",
      );
    } finally {
      setConnectingWalletId(null);
    }
  ...
  const signMint = useCallback(async () => {
    ...
    if (
      transactionPlan.sender &&
      transactionPlan.sender.toLowerCase() !== account.address.toLowerCase()
    ) {
      setWalletError(
        "The connected Sui wallet does not match the mint handoff sender.",
      );
      return;
    }
    ...
    setSigning(true);
    try {
      const transaction = buildMintTransactionFromPlan(
        transactionPlan,
        account.address,
      );
      ...
      if (receipt.status === "failure") {
        setWalletError(
          receipt.error || "The Sui wallet returned a failed transaction.",
        );
        return;
      }
      if (!receipt.objectId) {
        setWalletError(
          "The wallet returned a digest, but not the minted object id. Paste the object id below to record the receipt.",
        );
        return;
      }
      ...
      setWalletError(
        error instanceof Error
          ? error.message
          : "Could not sign mint transaction.",
      );
    } finally {
      setSigning(false);
    }
  ...
  const signListing = useCallback(async () => {
    ...
    if (
      ...
      setWalletError(
        "The connected Sui wallet does not match the listing handoff sender.",
      );
      return;
    }
    ...
    setSigning(true);
    try {
      const transaction = buildKioskListingTransactionFromPlan(
        transactionPlan,
        account.address,
      );
      const result = await suiDAppKit.signAndExecuteTransaction({
      ...
      if (receipt.status === "failure") {
        setWalletError(
          receipt.error ||
            "The Sui wallet returned a failed listing transaction.",
        );
        return;
      }
      ...
      setWalletError(
        error instanceof Error
          ? error.message
          : "Could not sign listing transaction.",
      );
    } finally {
      setSigning(false);
    }
