...
export default function BittensorPanel() {
  ...
  const askAgentAboutSubnet = async (subnet: BittensorSubnetSummary) => {
    const prompt = `Explain Bittensor subnet ${subnet.netuid} (${subnet.name}) for a Matterhorn Work user. Cover what it does, who benefits, current risks, and what actions are read-only versus signed.`;
    window.dispatchEvent(new CustomEvent("matterhorn:bittensor-agent-prompt", { detail: { prompt, netuid: subnet.netuid } }));
