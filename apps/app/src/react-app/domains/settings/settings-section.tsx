...
export function SettingsPill({ children, className }: SettingsPillProps) {
  return (
    <span
      className={cn(
        "inline-flex items-center gap-1.5 rounded-md bg-dls-hover/45 px-2.5 py-1 text-xs font-medium text-muted-foreground",
        className,
      )}
    >
      {children}
...
export function SettingsSectionHeaderTitle({ children, className }: SettingsItemHeaderTitleProps) {
  return (
    <h2 className={cn("flex items-center gap-2 text-base font-medium text-dls-text", className)}>
      {children}
    </h2>
  );
}
