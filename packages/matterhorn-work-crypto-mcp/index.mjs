...
async function bittensor_analyze_subnet(netuid) {
  const res = await callServer(`/api/bittensor/intelligence/subnet/${encodeURIComponent(String(netuid))}`);
  return { success: true, report: res.report, cards: res.cards || [] };
}

async function bittensor_analyze_wallet(ss58Address) {
  const res = await callServer(`/api/bittensor/intelligence/wallet/${encodeURIComponent(String(ss58Address || ""))}`);
  return { success: true, report: res.report, cards: res.cards || [] };
}
...
async function bittensor_export_subnet_adapter_marketplace(args = {}) {
  const params = new URLSearchParams();
  if (typeof args.adapter === "string") params.set("adapter", args.adapter);
  if (typeof args.serviceAdapter === "string" && !params.has("adapter")) params.set("serviceAdapter", args.serviceAdapter);
  if (Number.isFinite(args.netuid)) params.set("netuid", String(args.netuid));
  if (Number.isFinite(args.limit)) params.set("limit", String(args.limit));
  const query = params.toString();
  const res = await callServer(`/api/bittensor/adapters/marketplace-export${query ? `?${query}` : ""}`);
  return { success: true, marketplaceExport: res.marketplaceExport };
}

async function bittensor_plan_subnet_adapter_roadmap(args = {}) {
  const params = new URLSearchParams();
  if (typeof args.goal === "string") params.set("goal", args.goal);
  if (typeof args.query === "string" && !params.has("goal")) params.set("query", args.query);
  if (Number.isFinite(args.limit)) params.set("limit", String(args.limit));
  const query = params.toString();
  const res = await callServer(`/api/bittensor/adapters/roadmap${query ? `?${query}` : ""}`);
  return { success: true, roadmap: res.roadmap };
}

async function bittensor_export_subnet_adapter_roadmap(args = {}) {
  const params = new URLSearchParams();
  if (typeof args.goal === "string") params.set("goal", args.goal);
  if (typeof args.query === "string" && !params.has("goal")) params.set("query", args.query);
  if (Number.isFinite(args.limit)) params.set("limit", String(args.limit));
  const query = params.toString();
  const res = await callServer(`/api/bittensor/adapters/roadmap-export${query ? `?${query}` : ""}`);
  return { success: true, roadmapExport: res.roadmapExport };
}
...
async function bittensor_validate_subnet_adapter_manifest(args = {}) {
  const manifest = args.manifest && typeof args.manifest === "object" ? args.manifest : args;
  const res = await callServer("/api/bittensor/adapters/spec/validate", "POST", { manifest });
  return { success: res.success !== false, validation: res.validation, cards: res.cards || [] };
}

async function bittensor_get_subnet_adapter_manifest_examples(args = {}) {
  const params = new URLSearchParams();
  if (typeof args.adapter === "string") params.set("adapter", args.adapter);
  if (typeof args.serviceAdapter === "string" && !params.has("adapter")) params.set("serviceAdapter", args.serviceAdapter);
  if (Number.isFinite(args.netuid)) params.set("netuid", String(args.netuid));
  if (Number.isFinite(args.limit)) params.set("limit", String(args.limit));
  const query = params.toString();
  const res = await callServer(`/api/bittensor/adapters/spec/examples${query ? `?${query}` : ""}`);
  return { success: true, report: res.report, cards: res.cards || [] };
}

async function bittensor_validate_subnet_adapter_result(args = {}) {
  ...
  const res = await callServer("/api/bittensor/adapters/result/validate", "POST", body);
  return { success: res.success !== false, validation: res.validation, cards: res.cards || [] };
}
...
async function bittensor_export_subnet_adapter_preflight_packet(args = {}) {
  const body = {
    manifest: args.manifest,
    result: args.result,
  };
  if (Number.isFinite(args.maxResponseBytes)) body.maxResponseBytes = args.maxResponseBytes;
  const res = await callServer("/api/bittensor/adapters/preflight-export", "POST", body);
  return { success: res.success !== false, preflightExport: res.preflightExport };
}
...
async function bittensor_build_subnet_adapter_canary_packet(args = {}) {
  const params = new URLSearchParams();
  if (typeof args.adapter === "string") params.set("adapter", args.adapter);
  if (typeof args.serviceAdapter === "string" && !params.has("adapter")) params.set("serviceAdapter", args.serviceAdapter);
  if (Number.isFinite(args.netuid)) params.set("netuid", String(args.netuid));
  ...
  if (Number.isFinite(args.limit)) params.set("limit", String(args.limit));
  const query = params.toString();
  const res = await callServer(`/api/bittensor/adapters/canary-packet${query ? `?${query}` : ""}`);
  return { success: true, packet: res.packet, cards: res.cards || [] };
}

async function bittensor_export_subnet_adapter_canary_packet(args = {}) {
  const params = new URLSearchParams();
  if (typeof args.adapter === "string") params.set("adapter", args.adapter);
  if (typeof args.serviceAdapter === "string" && !params.has("adapter")) params.set("serviceAdapter", args.serviceAdapter);
  if (Number.isFinite(args.netuid)) params.set("netuid", String(args.netuid));
  ...
  if (Number.isFinite(args.limit)) params.set("limit", String(args.limit));
  const query = params.toString();
  const res = await callServer(`/api/bittensor/adapters/canary-packet-export${query ? `?${query}` : ""}`);
  return { success: true, canaryPacketExport: res.canaryPacketExport };
}

async function bittensor_build_subnet_adapter_canary_outcome_report(args = {}) {
  const res = await callServer("/api/bittensor/adapters/canary-outcome", "POST", {
    adapter: args.adapter,
    serviceAdapter: args.serviceAdapter,
    netuid: args.netuid,
    expectedRequestSha256: args.expectedRequestSha256 ?? args.requestSha256 ?? args.previewRequestSha256,
    result: args.result,
  });
  return { success: res.success !== false, report: res.report, cards: res.cards || [] };
}
...
async function bittensor_get_subnet_adapter_templates(args = {}) {
  ...
  if (Number.isFinite(args.netuid)) params.set("netuid", String(args.netuid));
  const query = params.toString();
  const res = await callServer(`/api/bittensor/adapters/templates${query ? `?${query}` : ""}`);
  return { success: true, report: res.report };
}

async function bittensor_get_subnet_adapter_candidates(args = {}) {
  const params = new URLSearchParams();
  if (typeof args.adapter === "string") params.set("adapter", args.adapter);
  if (Number.isFinite(args.netuid)) params.set("netuid", String(args.netuid));
  const query = params.toString();
  const res = await callServer(`/api/bittensor/adapters/candidates${query ? `?${query}` : ""}`);
  return { success: true, report: res.report };
}

async function bittensor_plan_subnet_adapter_onboarding(args = {}) {
  const params = new URLSearchParams();
  if (typeof args.adapter === "string") params.set("adapter", args.adapter);
  if (Number.isFinite(args.netuid)) params.set("netuid", String(args.netuid));
  if (Number.isFinite(args.limit)) params.set("limit", String(args.limit));
...
async function bittensor_get_subnet_adapter_canary_review(args = {}) {
  const params = new URLSearchParams();
  if (typeof args.adapter === "string") params.set("adapter", args.adapter);
  if (Number.isFinite(args.netuid)) params.set("netuid", String(args.netuid));
  const query = params.toString();
  const res = await callServer(`/api/bittensor/adapters/canary-review${query ? `?${query}` : ""}`);
  return { success: true, report: res.report };
}

async function bittensor_get_subnet_adapter_evidence_bundle(args = {}) {
  const params = new URLSearchParams();
  if (typeof args.adapter === "string") params.set("adapter", args.adapter);
  if (Number.isFinite(args.netuid)) params.set("netuid", String(args.netuid));
  if (Number.isFinite(args.limit)) params.set("limit", String(args.limit));
  const query = params.toString();
  const res = await callServer(`/api/bittensor/adapters/evidence-bundle${query ? `?${query}` : ""}`);
  return { success: true, report: res.report, cards: res.cards || [] };
}
...
async function bittensor_review_subnet_adapter_evidence(args = {}) {
  const params = new URLSearchParams();
  if (typeof args.adapter === "string") params.set("adapter", args.adapter);
  if (Number.isFinite(args.netuid)) params.set("netuid", String(args.netuid));
  if (Number.isFinite(args.limit)) params.set("limit", String(args.limit));
...
async function bittensor_export_subnet_adapter_conformance(args = {}) {
  const params = new URLSearchParams();
  if (Number.isFinite(args.netuid)) params.set("netuid", String(args.netuid));
  if (Number.isFinite(args.limit)) params.set("limit", String(args.limit));
  const query = params.toString();
  const res = await callServer(`/api/bittensor/adapters/conformance-export${query ? `?${query}` : ""}`);
  return { success: true, conformanceExport: res.conformanceExport };
}

async function bittensor_build_subnet_adapter_operator_handoff(args = {}) {
  const params = new URLSearchParams();
  if (typeof args.adapter === "string") params.set("adapter", args.adapter);
  if (Number.isFinite(args.netuid)) params.set("netuid", String(args.netuid));
  if (typeof args.task === "string") params.set("task", args.task);
  if (typeof args.ss58Address === "string") params.set("ss58Address", args.ss58Address);
  if (Number.isFinite(args.limit)) params.set("limit", String(args.limit));
  const query = params.toString();
...
async function bittensor_dry_run_subnet_adapters(args = {}) {
  ...
  if (Number.isFinite(args.limit)) params.set("limit", String(args.limit));
  const query = params.toString();
  const res = await callServer(`/api/bittensor/adapters/dry-run${query ? `?${query}` : ""}`);
  return { success: true, report: res.report };
}

async function bittensor_export_subnet_adapter_dry_run(args = {}) {
  const params = new URLSearchParams();
  if (Number.isFinite(args.netuid)) params.set("netuid", String(args.netuid));
  if (typeof args.task === "string") params.set("task", args.task);
  if (typeof args.ss58Address === "string") params.set("ss58Address", args.ss58Address);
  if (Number.isFinite(args.limit)) params.set("limit", String(args.limit));
  const query = params.toString();
  const res = await callServer(`/api/bittensor/adapters/dry-run-export${query ? `?${query}` : ""}`);
  return { success: true, dryRunExport: res.dryRunExport };
}
...
async function bittensor_prepare_extrinsic(args) {
  ...
  return {
    ...
    preview: res.preview,
    cards: res.cards || [],
    execution: "connected_wallet_review_required",
  };
}
...
async function bittensor_create_signing_handoff(args) {
  ...
  return {
    ...
    handoff: res.handoff,
    cards: res.cards || [],
    execution: "connected_wallet_handoff",
  };
}
...
const tools = [
  ...
  { name: "bittensor_chat", description: "Default first tool for ordinary Bittensor requests. Executes safe deterministic workflows for learn, discover, wallet reads, stake/unstake/transfer previews, capability-aware subnet use, validator comparison, monitoring, and follow-up prompts using public context.", inputSchema: { type: "object", properties: { message: { type: "string" }, contextId: { type: "string" }, context: { type: "object" }, ss58Address: { type: "string" }, netuid: { type: "number" }, amountTao: { type: "string" }, validatorHotkey: { type: "string" }, coldkey: { type: "string" }, recipient: { type: "string" }, destination: { type: "string" }, limit: { type: "number" }, strategy: { type: "string", enum: ["balanced", "yield", "safety"] }, rateTolerance: { type: "number" } }, required: ["message"] } },
  ...
  { name: "bittensor_get_subnet_capabilities", description: "Return the chat and service capability manifest for one subnet, or all subnets when netuid is omitted. Call this before previewing or invoking any direct subnet service.", inputSchema: { type: "object", properties: { netuid: { type: "number" } } } },
  { name: "bittensor_get_sidecar_status", description: "Report whether the configured Bittensor Subtensor sidecar can read and prepare wallet-reviewed actions. Submission is always disabled.", inputSchema: { type: "object", properties: {} } },
  ...
  { name: "bittensor_export_subnet_adapter_marketplace", description: "Export the read-only Bittensor subnet adapter marketplace as redacted markdown for operator handoff. Evidence only; omits endpoints, credentials, raw task text, wallet data, signing payloads, and full hashes.", inputSchema: { type: "object", properties: { adapter: { type: "string", enum: ["data_search", "inference", "compute", "creative_media", "agent_tooling"] }, serviceAdapter: { type: "string", enum: ["data_search", "inference", "compute", "creative_media", "agent_tooling"] }, netuid: { type: "number" }, limit: { type: "number" } } } },
  { name: "bittensor_plan_subnet_adapter_roadmap", description: "Plan the next safe Bittensor subnet service adapter work from marketplace status and an optional goal. Evidence only; does not configure, invoke, or approve subnet services.", inputSchema: { type: "object", properties: { goal: { type: "string" }, query: { type: "string" }, limit: { type: "number" } } } },
  { name: "bittensor_export_subnet_adapter_roadmap", description: "Export the safe Bittensor subnet adapter roadmap as redacted markdown for operator handoff. Evidence only; omits endpoints, credentials, raw task text, wallet data, signing payloads, and full hashes.", inputSchema: { type: "object", properties: { goal: { type: "string" }, query: { type: "string" }, limit: { type: "number" } } } },
  ...
  { name: "bittensor_export_subnet_adapter_preflight_packet", description: "Export a redacted markdown Bittensor adapter preflight packet for handoff. Omits raw manifest/result payloads and does not invoke subnet services.", inputSchema: { type: "object", properties: { manifest: { type: "object" }, result: { type: "object" }, maxResponseBytes: { type: "number" } }, required: ["manifest"] } },
  ...
  { name: "bittensor_export_subnet_adapter_canary_packet", description: "Export a redacted markdown Bittensor adapter canary packet for operator handoff. Omits full approval env values and does not invoke subnet services.", inputSchema: { type: "object", properties: { adapter: { type: "string", enum: ["data_search", "inference", "compute", "creative_media", "agent_tooling"] }, serviceAdapter: { type: "string", enum: ["data_search", "inference", "compute", "creative_media", "agent_tooling"] }, netuid: { type: "number" }, requestSha256: { type: "string" }, approvedBy: { type: "string" }, reason: { type: "string" }, ttlMinutes: { type: "number" }, limit: { type: "number" } } } },
  { name: "bittensor_build_subnet_adapter_canary_outcome_report", description: "Build a sanitized Bittensor adapter canary outcome report from an adapter result and expected preview request hash. Redacts full hashes, endpoints, task text, and secrets; does not invoke subnet services.", inputSchema: { type: "object", properties: { adapter: { type: "string", enum: ["data_search", "inference", "compute", "creative_media", "agent_tooling"] }, serviceAdapter: { type: "string", enum: ["data_search", "inference", "compute", "creative_media", "agent_tooling"] }, netuid: { type: "number" }, expectedRequestSha256: { type: "string" }, requestSha256: { type: "string" }, previewRequestSha256: { type: "string" }, result: { type: "object" } }, required: ["result"] } },
  ...
  { name: "bittensor_review_subnet_adapter_evidence", description: "Classify adapter evidence as blocked, mock-dry-run-ready, or manual real-canary-review-required. Does not authorize or invoke subnet services.", inputSchema: { type: "object", properties: { adapter: { type: "string", enum: ["data_search", "inference"] }, netuid: { type: "number" }, limit: { type: "number" } } } },
  ...
  { name: "bittensor_export_subnet_adapter_conformance", description: "Return a redacted markdown export for subnet adapter metadata conformance. Evidence only; it does not authorize or invoke real subnet services.", inputSchema: { type: "object", properties: { netuid: { type: "number" }, limit: { type: "number" } } } },
  { name: "bittensor_build_subnet_adapter_operator_handoff", description: "Return one redacted operator handoff packet combining adapter evidence review, evidence export, conformance export, and mock dry-run export. Evidence only; it does not authorize real subnet services.", inputSchema: { type: "object", properties: { adapter: { type: "string", enum: ["data_search", "inference"] }, netuid: { type: "number" }, task: { type: "string" }, ss58Address: { type: "string" }, limit: { type: "number" } } } },
  ...
  { name: "bittensor_prepare_extrinsic", description: "Prepare an unsigned Bittensor preview for exact connected-wallet review. Matterhorn cannot sign or broadcast it.", inputSchema: { type: "object", properties: { action: { type: "string", enum: ["stake", "unstake", "move_stake", "transfer", "set_child_hotkey", "register", "serve"] }, netuid: { type: "number" }, amountTao: { type: "string" }, coldkey: { type: "string" }, hotkey: { type: "string" }, destination: { type: "string" }, originNetuid: { type: "number" }, destinationNetuid: { type: "number" }, rateTolerance: { type: "number" } }, required: ["action"] } },
  { name: "bittensor_create_signing_handoff", description: "Create a checksummed handoff from an unsigned Bittensor preview for exact connected-wallet review. Matterhorn cannot sign or broadcast it.", inputSchema: { type: "object", properties: { preview: { type: "object" } }, required: ["preview"] } },
  ...
  switch (method) {
    ...
    case "tools/call": {
      ...
      switch (name) {
        ...
        case "bittensor_export_subnet_adapter_marketplace": return bittensor_export_subnet_adapter_marketplace(args).then(r => respond(textResult(r))).catch(catchErr);
        case "bittensor_plan_subnet_adapter_roadmap": return bittensor_plan_subnet_adapter_roadmap(args).then(r => respond(textResult(r))).catch(catchErr);
        case "bittensor_export_subnet_adapter_roadmap": return bittensor_export_subnet_adapter_roadmap(args).then(r => respond(textResult(r))).catch(catchErr);
        ...
        case "bittensor_export_subnet_adapter_preflight_packet": return bittensor_export_subnet_adapter_preflight_packet(args).then(r => respond(textResult(r))).catch(catchErr);
        ...
        case "bittensor_export_subnet_adapter_canary_packet": return bittensor_export_subnet_adapter_canary_packet(args).then(r => respond(textResult(r))).catch(catchErr);
        case "bittensor_build_subnet_adapter_canary_outcome_report": return bittensor_build_subnet_adapter_canary_outcome_report(args).then(r => respond(textResult(r))).catch(catchErr);
        ...
        case "bittensor_review_subnet_adapter_evidence": return bittensor_review_subnet_adapter_evidence(args).then(r => respond(textResult(r))).catch(catchErr);
        ...
        case "bittensor_export_subnet_adapter_conformance": return bittensor_export_subnet_adapter_conformance(args).then(r => respond(textResult(r))).catch(catchErr);
        case "bittensor_build_subnet_adapter_operator_handoff": return bittensor_build_subnet_adapter_operator_handoff(args).then(r => respond(textResult(r))).catch(catchErr);
        ...
        case "bittensor_export_subnet_adapter_dry_run": return bittensor_export_subnet_adapter_dry_run(args).then(r => respond(textResult(r))).catch(catchErr);
