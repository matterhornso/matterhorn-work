...
export const MATTERHORN_CRYPTO_ACTION_REGISTRY = [
  ...
  },
  {
    ...
    description: "Prepare a typed Bittensor, Hyperliquid, or Polymarket action for wallet review. Never signs or submits.",
    inputSchema: objectSchema({
      message: { type: "string", description: "Original user request." },
      venue: { type: "string", enum: ["auto", "bittensor", "hyperliquid", "polymarket"] },
      address: { type: "string", description: "Public 0x address." },
      ss58Address: { type: "string", description: "Public SS58 address." },
      coldkey: { type: "string", description: "Bittensor sender address." },
      recipient: { type: "string", description: "Bittensor recipient." },
      destination: { type: "string", description: "Bittensor destination." },
      netuid: { type: "number", description: "Bittensor subnet id." },
      validatorHotkey: { type: "string", description: "Bittensor validator hotkey." },
      amountTao: numberOrStringSchema,
      marketId: { type: "string", description: "Polymarket market id." },
      outcome: { type: "string", description: "Polymarket outcome." },
      asset: { type: "string", description: "Hyperliquid asset." },
      side: { type: "string", enum: ["buy", "sell", "long", "short", "yes", "no"] },
      size: numberOrStringSchema,
      price: numberOrStringSchema,
      orderType: { type: "string", enum: ["market", "limit"], description: "Market uses a mark; limit requires price." },
      network: { type: "string", enum: ["testnet", "mainnet"], description: "Wallet review network." },
