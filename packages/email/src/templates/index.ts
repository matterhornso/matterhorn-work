...
export const emailSubjects: { [Template in EmailTemplate]: (props: EmailTemplateProps[Template]) => string } = {
  verification: ({ verificationCode }) => `Your Matterhorn Desks verification code is ${verificationCode}`,
  passwordReset: () => "Reset your Matterhorn Desks password",
  organizationInvite: ({ organizationName }) => `You're invited to join ${organizationName} on Matterhorn Desks`,
  feedback: ({ name, source }) => `Matterhorn Desks feedback from ${name}${source ? ` (${source})` : ""}`,
}
