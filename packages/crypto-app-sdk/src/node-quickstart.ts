...
export type MatterhornCryptoAppQuickstartCliIo = {
...
export function matterhornCryptoAppQuickstartUsage(command = "create-matterhorn-crypto-app"): string {
...
function quickstartProtocol(
  value: string,
): MatterhornCryptoAppQuickstartProtocol | null {
  if (value === "sui" || value === "hyperliquid" || value === "bittensor") {
    return value;
  }
  return null;
}
...
/**
 * Runs the distributable quickstart without reading credentials or contacting
 * a network. Returning a status rather than exiting keeps the boundary
 * directly testable by both the repository wrapper and the published binary.
 */
export function runMatterhornCryptoAppQuickstartCli(
  argv: string[],
  io: MatterhornCryptoAppQuickstartCliIo = {
    stdout: (message) => console.log(message),
    stderr: (message) => console.error(message),
  },
  command = "create-matterhorn-crypto-app",
): number {
  ...
  try {
    ...
    io.stderr(matterhornCryptoAppQuickstartUsage(command));
    return 1;
  }
  if (args.help) {
    io.stdout(matterhornCryptoAppQuickstartUsage(command));
    return 0;
  }
  try {
    const protocol = quickstartProtocol(required(args.protocol, "--protocol"));
    if (!protocol) throw new Error("--protocol must be sui, hyperliquid, or bittensor");
    ...
    if (args.json) io.stdout(JSON.stringify(summary));
    else {
      io.stdout(`Created ${summary.appId} in ${summary.outputDirectory}`);
      io.stdout(`Local checks: passed (${summary.network}, read-only)`);
      io.stdout("Next: open README.md in the generated directory.");
    }
    return 0;
  } catch (error) {
    const issues = error instanceof MatterhornCryptoAppQuickstartError
      ? error.issues
      : [];
    io.stderr(error instanceof Error ? error.message : "Quickstart failed");
    for (const issue of issues) io.stderr(`- ${issue}`);
    return 1;
  }
}
