...
### Sixth packet: final agent-path and readiness checks

- At 19:20 UTC all 11 remote checks passed on `379153d3db`. Ran 76 isolated agent/session, selected-memory, model-routing, privacy-policy and token-budget tests before the final fixes; all passed. These use a mock engine and are not real model responses.
- Found silent truncation of selected Memory IDs before the existing 32-record limit check. Removed that truncation. A new HTTP regression verifies both `memoryIds` and `selectedMemoryIds`, on preflight and submission, return the explicit 400 limit error before provider dispatch. The regression failed before the fix (404 lookup after truncation); all 77 agent-path tests now pass.
- Added these agent/session, routing, privacy and token-budget tests to the release safety gate, with required-file checks in its wiring contract.
- Found backup configuration readiness omitted the uploader's required region. A regression reproduced false readiness with only `AWS_SES_REGION`. Readiness now requires `AWS_REGION` or `AWS_DEFAULT_REGION`, matching the uploader. Eight focused backup/launch tests pass, including whitespace and fallback-region cases. The deployment guide records this requirement.
- Server typecheck, gate-wiring contract, production configuration contract, zero-finding secret scan and a fresh full safety-gate run passed on the finalized changes. An earlier in-flight gate encountered the intentionally failing region regression before implementation; the final completed run is green.
- Read-only hosted strict probe passed again for deployed commit `ed3ee445e4cb572d65da041dab59c1f413502f50`, guarded mode off and signup paused. Fresh `/health/launch` returned HTTP 503 / `not_ready`: email delivery/events/transport, password reset and backup configuration/freshness remain false. Infrastructure, legal, usage, privacy-policy and inference **configuration** checks are true; none proves live inference or email delivery.
- Logs: `/tmp/mh-solo-{final-agent-paths,final-hosted-probe,memory-limit-before,memory-limit-after,memory-limit-agent-paths,backup-region-before,backup-region-after,final-typecheck,final-launch-contract,final-safety}.log`.
- Feature work is now stopped early to leave room for final CI review and reporting. Do not merge or deploy this draft. No new implementation after the original 20:07 UTC deadline; pause the heartbeat then.
