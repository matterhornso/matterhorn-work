...
- Phase 5 SDK published-provenance verifier: complete in code. An operator can verify one exact public-registry SDK version against npm registry signatures, transparency-backed publish and SLSA provenance attestations, the fixed Matterhorn repository and workflow, and one exact source commit. Lifecycle scripts are disabled, ambient npm credentials are not inherited, raw attestations are not emitted, and the command has no publication or promotion authority. Publishing the SDK and supplying live provenance evidence remain explicit operator release steps.
