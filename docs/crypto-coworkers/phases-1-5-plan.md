...
- Current verification evidence: 987 application tests and 1,256 server tests pass, including the four-role template, master-prompt budget, tenant isolation, wallet-only escalation, cross-account wallet-intent cancellation denial, and exact wallet-review lifecycle contracts. The full 10-stage platform safety gate, application and server type checks, shared-types build, secret scan, production web build, and task-first bundle budget also pass. Current rendered desktop acceptance confirms exact wallet activity, progressive disclosure, receipt verification state, expiry, and the safe cancellation dialog. Earlier rendered desktop and 375 px mobile acceptance confirmed all four creation choices, Treasury Coworker creation, exact authority and limits, zero horizontal overflow, zero unnamed buttons, lifecycle controls, chat binding, reload persistence, Agent Files continuity, and zero browser console warnings or errors.
