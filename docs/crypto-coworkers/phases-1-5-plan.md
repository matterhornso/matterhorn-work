...
- Guarded final-provider-message boundary: complete in code. The pinned OpenCode `experimental.chat.messages.transform` hook runs in Matterhorn's final managed plugin and sends the exact final user/assistant parts and tool results only to a runtime-authenticated internal validator. The validator enforces one tenant/session/run, bounds count and bytes, scans the complete array for secrets added after initial preflight, rechecks the exact provider privacy policy, and stores only a transient digest. The following provider-system release consumes that proof once within 30 seconds; retries and tool continuations must validate their newly assembled arrays again. OpenCode's provider-backed title agent is disabled because upstream forks it before the final-message hook and could otherwise race for the one-shot proof; hosted chat names stay deterministic and user-editable. Mixed-session, oversized, malformed, replayed, restarted, policy-drifted, secret-bearing, or pre-hook side-channel requests fail before provider contact. The compatibility manifest pins both OpenCode privacy hooks, title-agent disablement, and the guard's last-plugin ordering. Live hosted acceptance remains pending.
