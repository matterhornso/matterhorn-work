...
- Phase 2 first-run coworker guidance: complete for the invite UI slice. Each coworker now exposes one contextual next action—reload setup, connect an app, review access, or start chat—rather than a disabled unexplained button. Resource selection is named by what the coworker can use, numeric operating limits are progressively disclosed under `Safety limits`, and permanent connection/coworker actions use plain removal or disabling language. The exact wallet-review, connected-wallet-only submission, private-data, and no-key boundaries remain visible and unchanged.
