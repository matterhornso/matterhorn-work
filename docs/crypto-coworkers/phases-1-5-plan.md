...
- Guarded external MCP evidence: generated Matterhorn Skill, Codex, Claude Code, and generic MCP setup selects the `guarded_client` profile. Both the configuration CLI and server-control MCP now fail safe to that profile when no profile is specified. The profile exposes exactly 11 workspace-session tools, rejects hidden tool calls before network access, fails startup on unknown profile names, and reduces the serialized model-facing tool schema by more than 80% relative to the legacy full registry. Its prompt schema and runtime reject client-supplied system instructions, tool overrides, compatibility agent/provider aliases, unknown fields, and one-request consent bearer values before server contact; the authoritative server continues to construct and intersect those policies. Trusted operators must select `server` or `full` explicitly; host approval calls still require separate host authority.
