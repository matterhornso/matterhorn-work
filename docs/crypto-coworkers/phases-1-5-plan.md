...
- Public-evidence context continuity: complete in code. Certified adapter results now carry the same exact projection and observation proofs reconciled into their run receipt. Every direct model-facing certified read independently verifies those proofs before returning data, and every scheduled watch verifies them before marking its model-free guarded run successful. Coworker watch alerts then bind to the observation proof—which itself binds the projection, app, revision, action, network, source, block/version, and observation time—instead of the older raw-response digest. The exported wallet-intent compiler itself independently recomputes both proofs before compiling a financial result into a wallet review, so a direct or future caller cannot bypass the transaction service's verification boundary. Proof-less legacy results remain display-compatible but cannot create a new wallet intent, while malformed or mutated app, revision, action, network, source, block, observation-time, or typed-result evidence fails closed. Later crypto chats retain only these content-free proof references; legacy source and freshness strings are domain-separated and hashed before reuse, while partial or malformed modern proofs are discarded or rejected rather than entering coworker state. No query, result body, external instruction text, wallet identity, credential, or tenant identity is replayed. Current regression evidence is 1,073 application tests, 1,555 server tests, and 36 Crypto App SDK tests with zero failures, plus the complete 10-stage platform safety gate.
