...
- Phase 1 restricted JSON-RPC 2.0 transport: complete in code and backend-only. Each authorized call uses the signed action ID as its exact method, includes only the authorized network and validated arguments, binds the response to a fresh request ID, accepts only a closed evidence result, and rejects batches, notifications, peer requests, extra fields, injected error details, and upstream cost claims. Live sealed certification and operator promotion remain pending.
- Phase 1 restricted OpenAPI action profile: complete in code and backend-only. The signed manifest binds the adapter origin, profile version, and one static POST path per action. Legacy unprofiled manifests remain parseable but cannot pass conformance or execute. Dynamic documents, remote references, server lists, callbacks, links, redirects, queries, caller-selected operations, and upstream cost claims are excluded. Live sealed certification and operator promotion remain pending.
...
- Guarded provider-system boundary: complete in code. The authoritative gateway now retains the exact classified agent and Matterhorn system sections only in memory for the active run. The last managed OpenCode plugin replaces all late OpenCode environment, skill, MCP, workspace, and provider system additions with those hash-verified bytes, scoped to the exact tenant, session, provider, model, and message-or-compaction purpose. Failure blocks provider contact in every guarded-tool rollout mode. Automatic OpenCode compaction is disabled; only the privacy- and allowance-protected workspace compaction route may summarize a hosted chat. Live hosted acceptance remains pending.
- Guarded session-history privacy boundary: complete in code. Every message preflight and dispatch hashes and scans the exact stored chat, re-reads it immediately before starting the guarded run, and invalidates consent after any stored-byte change. A content-free, tenant/session-scoped privacy floor can only increase from public research to private workspace to transaction; legacy history without a trustworthy floor is private by default, stored secret material blocks provider contact, and oversized history fails closed with an explicit compact-or-new-chat recovery path. Chat deletion and workspace purge remove the floor, while normal expiry is capped at 365 days. Live hosted acceptance remains pending.
- Guarded final-provider-message boundary: complete in code. The pinned OpenCode `experimental.chat.messages.transform` hook runs in Matterhorn's final managed plugin and sends the exact final user/assistant parts and tool results only to a runtime-authenticated internal validator. The validator enforces one tenant/session/run, bounds count and bytes, scans the complete array for secrets, private attachments, and wallet/transaction intent added after initial preflight, marks final tool output as untrusted external data, rechecks the exact provider privacy policy, and stores only a transient digest. The following provider-system release consumes that proof once within 30 seconds; retries and tool continuations must validate their newly assembled arrays again. OpenCode's provider-backed title agent is disabled because upstream forks it before the final-message hook and could otherwise race for the one-shot proof; hosted chat names stay deterministic and user-editable. Mixed-session, oversized, malformed, replayed, restarted, policy-drifted, sensitivity-increased, secret-bearing, or pre-hook side-channel requests fail before provider contact. The compatibility manifest pins both OpenCode privacy hooks, title-agent disablement, and the guard's last-plugin ordering. Live hosted acceptance remains pending.
...
- Token-aware context observability: complete in code. Each guarded response receipt now records bounded, content-free compiler measurements: exact compiler version, system/policy/data character counts, active-versus-available crypto tool counts and schema characters, and included/shortened/omitted data-section counts. The browser explains the narrowed action set in plain language and keeps exact compiler measurements under technical details. No prompt, Memory, file, tool argument, tool result, secret, wallet identifier, or account identifier is retained in these metrics. Provider-reported input/output/cache tokens remain the authoritative usage measurement; the live 40% reduction gate still requires hosted baseline and candidate evidence for the exact release.
- Certified public evidence cache: complete in code. The adapter router can reuse only successful, typed, quarantined, explicitly informational, unauthenticated, scope-free reads whose signed action declares `block_bound_public`, carries a current block or version, and remains inside the certified action's freshness window. Absence or an unknown policy fails closed to a live call, while static conformance rejects cache declarations on private, financial, authenticated, scoped, or freshness-unbounded actions. Cache identity binds the workspace, connection revision and grants, signed manifest and certification hashes, policy version, action, network, projected schema, and canonical argument hash; only the query hash is retained. Every hit still requires a fresh exact-run capability and receives a new usage reconciliation record, while DNS and upstream adapter traffic are skipped and charged at zero tool cost. New results and run receipts distinguish live delivery from certified-cache reuse, bind bounded observation age and freshness limits into encrypted evidence, and keep the query and raw result out of the receipt. Private or wallet-linked data, authenticated connections, watch/prepare/simulate actions, unbound observations, expired evidence, paused/revoked connections, and changed certification identities never use the cache. First-party manifest revisions were advanced for public Hyperliquid, Bittensor, and Polymarket reads, so prior certification cannot silently authorize the new policy. Entries are in-memory, defensively copied, size-bounded, count-bounded, and fail safely back to a live read when admission is unavailable.
...
- Public-evidence receipt identity: complete in code. Every new live or certified-cache result now adds two domain-separated, content-free SHA-256 proofs to the guarded run receipt: one for the exact tenant connection, action access class, and typed quarantined model-facing projection, and one for the app, revision, connection, action, access, network, source, observation time, block or version, and projection proof. Live and cached reuse through the same connection retains the same observation identity while changed evidence, source metadata, app revision, tenant connection, action authority, action, or network cannot. The strict reconciliation boundary rejects missing, malformed, open, or forged proof metadata before consuming the reservation. Queries, raw results, wallet identifiers, upstream bodies, capability values, and secrets remain absent; encrypted Walrus evidence binds both proofs through the existing closed evidence hashes. The primary UI stays plain-language, while exact proofs are available only inside technical response details. Earlier receipts without these additive fields remain readable but cannot authorize new work. Current regression evidence is 1,073 application tests, 1,555 server tests, and 36 Crypto App SDK tests with zero failures.
- Public-evidence context continuity: complete in code. Certified adapter results now carry the same exact projection and observation proofs reconciled into their run receipt. Every direct model-facing certified read independently verifies those proofs before returning data. Both the guarded watch executor and the exported watch runner verify them before a model-free guarded run can succeed, establish a baseline, or create an alert, so an alternate executor cannot restore the legacy compatibility path. Coworker watch alerts then bind to the observation proof—which itself binds the projection, app, revision, tenant connection, action, access class, network, source, block/version, and observation time—instead of the older raw-response digest. The exported wallet-intent compiler itself independently recomputes both proofs before compiling a financial result into a wallet review, so a direct or future caller cannot bypass the transaction service's verification boundary. Proof-less legacy results remain display-compatible but cannot create a new scheduled result or wallet intent, while malformed or mutated app, revision, tenant connection, action, access class, network, source, block, observation-time, or typed-result evidence fails closed. Later crypto chats retain only these content-free proof references; legacy source and freshness strings are domain-separated and hashed before reuse, while partial or malformed modern proofs are discarded or rejected rather than entering coworker state. No query, result body, external instruction text, wallet identity, credential, or tenant identity is replayed. Current regression evidence is 1,073 application tests, 1,555 server tests, and 36 Crypto App SDK tests with zero failures, plus the complete 10-stage platform safety gate.
- Provider-visible crypto result minimization: complete in code. The certified executor keeps tenant connection IDs, operational reservation IDs, evidence references, exact proof hashes, guarded run IDs, pending-intent IDs and revisions, and policy/intent hashes inside Matterhorn. Direct reads expose only the app, action, network, typed quarantined result, source/freshness information, and the untrusted-data label. Financial tools expose only the human-readable policy outcome and exact wallet-review terms needed to explain the action. The authoritative adapter envelope is consumed server-side, its exact content-free proofs remain in the tenant receipt, and the full reviewed-action v2 plus pending intent remain in the tenant wallet airlock. The model is told that the connected wallet must approve and submit, while receiving no signing or submission authority.
...
- Phase 1 Polymarket public-read boundaries and certification workflow: complete in code and backend-only for active-market discovery plus exact-token order-book evidence. Gamma discovery and CLOB reads use separate signed manifests, separate pinned HTTPS origins, bodyless GETs, canonical uint256 token IDs, closed typed projections, bounded sorted levels, and read-only coworker capabilities. A distinct operator command accepts only the exact first-party IDs, revisions, actions, origins, public network, and informational read authority, then emits hash-only `certified_mainnet` evidence after every adversarial probe passes. Account, compliance, order, cancellation, signing, relay, submission, credentials, and destination overrides are excluded. The separate wallet UI now asks Polymarket's documented geoblock endpoint directly from the user's browser before preparing a new order and repeats that exact-origin, credential-free, no-store check immediately before loading the CLOB client. The reported IP is validated and discarded; a blocked, malformed, redirected, oversized, timed-out, or unavailable response fails closed before wallet authorization. Cancellations remain available so a close-only user is not trapped in an open order. This browser check does not certify coworker transaction preparation or let the server infer user eligibility. Operator-signed read manifests, live sealed reports, registry insertion, and promotion remain pending; the gateway stays off by default.
- Phase 1 trusted Polymarket jurisdiction and wallet-preview boundary: complete in code and dormant. The Vercel same-origin function removes caller-supplied Matterhorn and Vercel geo headers before forwarding, then may create one 60-second HMAC proof from Vercel's country, region, and anti-spoofed client-IP signals. The proof is bound to the exact method, normalized backend route, hashed peer, and hashed Vercel request identity. The backend accepts it only with the independent trusted-proxy secret and rejects changed, stale, future, open, malformed, wrong-route, wrong-method, wrong-peer, or direct-backend claims. A closed, server-owned policy reproduces the official Polymarket country and subregion restrictions, denies close-only and frontend-restricted locations from opening positions, requires a region where the official policy is region-specific, and fails closed after its explicit 2026-10-04 review deadline. Its content-free decision is bound to the exact evidence, guarded run, and single-use capability and expires no later than the 60-second edge proof. Country, region, IP, and Vercel request ID never enter provider messages or security receipts. Missing or invalid proof does not impede public research but grants no preparation authority. Because Polymarket's browser geoblock response is unsigned, it can only add a denial and can never override the server policy. The certified wallet-preview adapter performs a credential-free exact-token CLOB depth simulation, binds signer, condition, token, side-specific amount, FAK limit, tick size, negative-risk mode, slippage, policy, freshness, and expiry into reviewed-action v2, and leaves authentication, signing, and submission exclusively to the connected Polygon wallet. The managed OpenCode bridge routes this exact contract only through the certified coworker executor and fails closed instead of falling back to the incompatible legacy handoff route. Live sealed certification, registry insertion, promotion, and hosted acceptance remain explicit operator actions; the gateway stays off by default.
...
- Phase 2 chat and authority binding: complete for all four initial roles. Market Analyst and Risk Monitor remain read/watch only; Transaction Coordinator and Treasury Coworker may prepare one exact testnet wallet review per action family but receive no signing or submission authority. Selected profiles become versioned private system context. A concise server-owned, versioned master prompt constrains each role and accepts financial intent only from the user's current direct request; files, Memory, app/MCP output, and prior actions remain untrusted data rather than instructions, consent, or transaction authority. Guarded run grants and hidden capabilities bind the exact coworker revision/policy, certified app/action/proxy mapping, networks, data labels, authorities, and per-run budgets. Legacy direct tools, broadened scopes, disallowed private context, stale profiles, and disallowed provider consent fail closed, while lifecycle changes revoke staged authority immediately.
...
- Phase 2 trusted-runtime system-context binding: complete. Direct local
  OpenCode prompts place the exact final forwarded system bytes inside the same
  secret scan, sensitivity classification, request hash, and single-use consent
  boundary before allowance reservation or provider contact. Workflow paths and
  environment metadata are workspace-private, while an address or balance linked
  to a Matterhorn account is wallet-private even though its underlying chain data
  is public. Hosted clients remain unable to supply system context.
- Phase 2 opaque-command privacy boundary: complete. OpenCode command templates
  and their implicit runtime context are expanded upstream after Matterhorn's
  request parser, so their exact provider-bound bytes cannot be proven by the
  normal one-request consent hash. Trusted/local command dispatch therefore
  fails before abort, usage reservation, guarded-run creation, or provider
  contact unless the selected model is processed locally or has a current
  verified no-training policy. Users can send the same instruction through the
  normal authoritative chat gateway when they need disclosed public research
  through an unverified provider. Hosted accounts cannot invoke raw commands.
- Phase 2 selected-agent privacy binding: complete. Matterhorn resolves the
  effective OpenCode session agent before preflight and binds its exact prompt
  bytes, digest, and agent id into secret scanning, sensitivity classification,
  and one-request consent. Exact shipped Matterhorn prompts remain public
  platform policy; modified or custom workspace prompts are private. Stable and
  trusted prompt paths send the resolved id explicitly and re-read the agent
  immediately before dispatch, so an implicit session-agent substitution,
  one-byte prompt change, or secret-bearing agent file fails without provider
  traffic. Trusted commands enforce the same agent-prompt scan in addition to
  their local-or-verified-provider restriction.
...
- Phase 2 private-mode UI proof binding: complete. The account model-policy response carries the exact Venice catalog-proof expiry, and the composer can show `Private on` only while the server proof is current. The toggle chooses the first connected model in the exact current proof rather than trusting provider-catalog order, so an unverified leading entry during a rolling update cannot hide another valid private model. An empty intersection, missing rolling-upgrade metadata, expiry, refresh failure, a removed model, or an unverified provider fails closed in both the UI and the authoritative pre-dispatch check. Rendered acceptance covers absent, setup, proof-unavailable, verified-off, verified-on, and busy states; proof failures remain ordinary buttons rather than switches and expose the exact recovery reason to assistive technology.
- Phase 2 Venice hosted-acceptance companion: complete in code and dormant. An additive owner-only template and verifier bind matching frontend/backend commits, the current exact Venice proof, all visible and keyboard toggle states, private-workspace dispatch, zero-retention receipts, proof-expiry and model-substitution denials, secret zero-contact/zero-usage behavior, reload, and two-account isolation to three distinct redacted content-addressed reports. It changes neither the existing Phase 1–5 v2 schema nor any rollout flag, contacts no provider, and is not a substitute for operator-controlled hosted evidence. Signed-report strict-gate integration remains a separately approved migration.
...
- Phase 2 one-active-run boundary: complete. Every account-facing message and compact request, plus trusted/local raw prompt, message, command, and summarize dispatch, must stop the current OpenCode response before a replacement can start. Abort failure returns one stable no-dispatch error. A newly accepted request receives a server-owned message ID and a fresh guarded run binding before provider contact, so caller-selected IDs and late events from the replaced run cannot inherit the new run's authority, consent, quota, capability, or receipt.
...
- Phase 2 model-context frame hardening: complete. The versioned server compiler places user-controlled profile, state, Memory, and Agent File data before the complete authoritative policy suffix, and deterministically escapes any data line that imitates Matterhorn's reserved policy headings or data delimiters before hashing the exact provider-bound context. Benign prose remains intact; forged frames cannot visually create a second policy block or occupy the final instruction position.
...
- Phase 2 first-run coworker guidance: complete for the invite UI slice. Home asks for the outcome before presenting role decisions, shows one deterministic coworker suggestion only after the user types, and keeps all four role explanations behind an ordinary Change control. Each coworker then exposes one contextual next action—try again, choose an app, choose access, or start chat—rather than a disabled unexplained button. Resource selection is named as apps and information, asks for one app before optional private information, and says that nothing is shared until the user saves. A missing app produces an explicit live status plus a disabled `Choose an app above` control; selecting one changes that same control to `Save and continue`, and persistence prevents duplicate submission. Numeric operating limits are progressively disclosed under `Limits`, and permanent connection/coworker actions use plain removal or disabling language. Empty wallet, check, and update sections collapse into one plain-language Activity summary, while pause and permanent-disable controls stay behind a separate deliberate disclosure. Existing work opens automatically, so an active wallet review or alert is never hidden. The exact wallet-review, connected-wallet-only submission, private-data, and no-key boundaries remain visible and unchanged.
- Phase 2 direct Coworkers activation guidance: complete for the empty panel. A first-time user now sees what each coworker job does before choosing, receives one explicit instruction to continue, and no longer sees an arbitrarily emphasized default. Creating a coworker still leads to explicit app, file, and memory review; wallet signing and submission authority are unchanged.
- Phase 2 Coworker terminology: complete for the first-run and control surfaces. Both now use the same four action names—Research markets, Watch risk, Prepare a wallet review, and Track balances—without changing persisted role IDs, server templates, or authority.
...
- Phase 3 shared transaction contract boundary: complete. The public desk contract no longer contains a Matterhorn-after-signature authority. Every in-app financial completion must declare `connected_wallet` for both its support level and submission authority, and the contract builder rejects mismatched authority pairs before a desk registry can load. Agents, servers, schedulers, MCP clients, and CLI clients remain unable to sign or submit.
...
- Phase 2 account Coworkers control center: complete for the current internal-mode slice. Users can create, select, start a chat with, pause, resume, and revoke any of the four initial coworkers through plain-language controls: Research markets, Watch risk, Prepare a wallet review, or Track balances. Home now begins with one outcome field, makes a deterministic local coworker suggestion, permits an explicit user override, and carries the unsent outcome into the selected coworker's chat only after resource-access review. First-run access keeps the required app choice visible, moves optional private files and saved memory behind one disclosure, prevents continuation without an explicitly selected app, and opens the unsent chat immediately after the saved access grant succeeds. The panel shows automatic authority, approval boundaries, prohibited authority, request limits, watches, alerts, and wallet-review work without exposing signing or submission controls. A content-free, owner-scoped summary reports only unread counts and their latest timestamps, so the picker identifies coworkers with new updates and one flat attention row opens the latest coworker's Activity section; alert titles, summaries, evidence, wallet data, and owner IDs are excluded from this aggregate. The same bounded summary now appears as a quiet unread count on the existing desktop and mobile Coworkers navigation, shares the panel query cache, and clears immediately after updates are read without exposing update content outside the panel. Users can also inspect the bounded decisions, positions, open risks, pending actions, and source counts carried into future chats, then clear that structured state through one revision-checked confirmation without deleting chats, files, Memory, app access, or wallet history. Raw evidence hashes remain out of the primary UI. Wallet activity distinguishes active reviews from terminal history, exposes the exact operation, amount, network, signer, recipient, simulation, policy checks, expiry, and public-receipt verification state, and permits only a confirmed pre-submission cancellation. Starting or selecting a coworker now creates a durable, tenant-scoped server binding to the exact coworker and resource revisions. The binding survives reloads, closed browsers, and another signed-in device; preflight and send resolve it independently and reject caller substitution, stale app access, replayed revisions, and cross-workspace or cross-account use. Clearing the coworker removes the server binding before the local display context, so the UI cannot claim authority changed when the authoritative state did not. The binding contains no prompt, file, Memory, credential, wallet, or transaction content. On compact screens, the surrounding sheet provides the one visible Coworkers title while the panel preserves a screen-reader heading and a concise access reminder, avoiding duplicated mobile headers.
- Phase 2 coworker chat-fork continuity: complete for the current internal-mode slice. Forking a coworker chat can inherit only the source chat's exact active assignment through a same-tenant server operation. The caller cannot select a different coworker, change access, overwrite the target, replay inheritance, or copy a stale assignment. The fork remains reachable and shows a plain warning if the post-fork assignment cannot be preserved; no transaction, signing, or submission authority is added.
- Phase 2 coworker chat-deletion cleanup: complete for the current internal-mode slice. Deleting a chat through the authoritative workspace route also removes only that tenant's durable coworker assignment. Cleanup is idempotent, can finish after an already-accepted upstream deletion, and remains available after coworker invite access is revoked so revocation cannot strand private assignment metadata. Other chats, coworkers, resources, and tenants are unchanged.
- Phase 2 per-chat model privacy choice: complete for the current app slice. Choosing a model or turning on Venice Private changes only the active workspace chat instead of silently replacing the user's default for every chat. The exact model and reasoning choice survive reload, are inherited only by an explicit chat fork, and are removed with the deleted chat. Immediate desk tasks bind the model they actually dispatch so the restored UI cannot claim a different provider or privacy mode.
- Phase 2 coworker resource sandbox: complete for the current internal-mode slice. Each coworker has one optimistic-revisioned, owner/workspace-scoped resource grant containing only explicitly selected Agent File revisions, Memory record versions, and certified app connections/actions/networks. No grant means no app authority. Matterhorn can now produce an advisory, content-free access recommendation from files already assigned to the coworker, topic-tagged Memory, and current certified app connections inside the fixed profile. Recommendations never activate themselves: the server binds each hash to the tenant, profile and scope revisions, resource hashes, app revisions, exact actions, and networks, recomputes it before acceptance, and rejects changed IDs or stale inputs. The authoritative message gateway resolves and hash-binds the exact current resources, forces private-workspace handling when private files or Memory are selected, forbids unverified-provider consent, and rejects deleted, changed, disconnected, or cross-tenant resources. The account UI keeps chat disabled until the grant is current and includes an app connection, loads optional app, file, and Memory services independently, explains any unavailable service in plain language without sending the user into a dead setup path, and sends IDs and revisions rather than resource contents or privacy overrides.
- Phase 2 plain-language run receipts: complete. Every completed response leads with understandable privacy, time, token, app-data, and wallet facts. Secret-class data is excluded from the shared-data list and identified only as blocked before sharing. One-request consent and connected-wallet-only submission are explicit. Request, receipt, authorization, app-call, access, trust, simulation, policy, and public-chain proofs remain available through a separate technical disclosure without exposing prompts, arguments, credentials, signatures, or capability tokens.
- Phase 2 plain-language wallet review: complete for the current connected-wallet slice. Review screens lead with amount, recipient, asset changes, network, and the safety result; the primary action says that it continues in the user's wallet. Contract bytes, network check identifiers, and public receipt identifiers remain available only through deliberate technical disclosure. Coworker wallet activity uses the same hierarchy, while exact safety terms and connected-wallet-only signing remain unchanged.
- Phase 4 Agent Files account UI: complete for the current testnet slice. Users can add files, choose up to eight, bind them and their owning coworker to a new or existing chat, and continue with the same privacy-bound coworker context. The primary flow uses plain file and backup language; Walrus storage periods, public-testnet persistence, WAL fees, recovery-key destruction, and the connected-wallet-only renewal boundary remain visible through deliberate disclosure at the exact approval step. The file panel follows the active session coworker rather than silently switching tenants or identities.
- Phase 4 Agent Files secret-format boundary: complete. Upload and pre-context scans reject common unlabeled crypto key and wallet-export encodings, including bare 32-byte keys, Sui private-key strings and legacy entries, Bitcoin wallet-import keys, extended private keys, Solana byte-array exports, private JWKs, encrypted Ethereum keystores, and private-key PEM blocks. Public hashes, addresses, and transaction signatures remain available when explicitly identified as public evidence. Rechecking before model-context compilation prevents a legacy stored descriptor from bypassing the current detector.
- Phase 2 Memory secret-format boundary: complete. Shared validation now rejects the same common unlabeled crypto key and wallet-export encodings across the exact model-facing title, summary, and body before capture, suggestion acceptance, browser context storage, export, or chat use. The authoritative gateway rescans selected records immediately before privacy preflight and provider dispatch, preventing legacy or restored content from bypassing newer rules. Explicitly named public hashes, digests, addresses, accounts, checksums, and public keys remain usable as evidence; safety errors never echo the rejected value.
- Phase 2 persistent coworker secret-format boundary: complete. Model-facing profile text, structured working state, watch definitions and returned condition values, and inbox items are scanned before storage or state transition. Restored profile, state, watch, and inbox rows are rescanned before account access, scheduling, or model-context compilation, so legacy data cannot bypass newer crypto-key detectors. Corrupt rows fail closed without an unvalidated fallback.
...
- Current verification evidence: 1,073 application tests, 1,555 server tests, and 36 Crypto App SDK tests pass, including the four-role template, master-prompt budget, tenant isolation, wallet-only escalation, cross-account wallet-intent cancellation and receipt denial, exact wallet-review lifecycle contracts, exact certified evidence proof verification before direct model reads, scheduled-run success, transaction-service use, and the exported wallet-intent compiler boundary, Bittensor test/Finney/local network separation, developer-database migration, policy-pinned passed/failed certification outcomes, privacy-safe app-revision usage aggregation and ownership enforcement, signed public-cache opt-in and private/authenticated cache exclusion, composite action-authority and non-finite timing rejection, nested secret/signing schema and metadata/literal rejection before signing or adapter invocation, global schema/value traversal-budget enforcement, oversized input rejection, raw-evidence rejection, one-shot consent and message handoff binding, mutation rejection, stale/expired behavior, crash-safe consent, message, run startup, dispatch readiness, intent replacement, exact trusted-runtime system-context binding, fail-closed opaque command expansion, exact selected-agent instruction binding, and exact hidden compaction-agent binding, exact-revision evidence publication, wallet-owned testnet renewal and deletion, key destruction, operation serialization, the separate certified Polymarket discovery/CLOB read boundaries, trusted edge-jurisdiction attestation and consent binding, format-aware Agent File, Memory, and persistent coworker secret exclusion, and rendered progressive-disclosure wallet review. The resource-sandbox and persistent-content slices add 68 focused application/server checks for tenant isolation, exact revision binding, stale-resource invalidation, content-free recommendations, private-profile compatibility, zero-authority defaults, partial-service degradation, minimal account requests, common unlabeled crypto key formats, public-evidence preservation, pre-context and restored-row rescanning, and bounded fail-closed Memory and coworker-content scanning. Focused certification and CLI tests additionally prove the public-read scope, exact origin/revision/action lock, sealed output, secret-input rejection, and absence of transaction authority. The full 10-stage platform safety gate, application and server type checks, server and SDK builds, OpenWork 0.18.42 / OpenCode 1.18.27 compatibility gate, strict secret scan, dependency audit, production web build, and task-first bundle budget pass. Rendered 1440 px desktop and 390 px mobile acceptance confirms keyboard operation, no unnamed controls, no horizontal overflow, deterministic local coworker suggestion, explicit user override, outcome continuity, exact resource selection, and a disabled chat start until a current app-scoped grant exists. Earlier rendered acceptance confirmed that the desktop workspace rail remains visible while a compact mobile menu exposes Coworkers, Files, MCPs, Memory, Notes, Wallet, Profile, and every crypto desk; a coworker row opens the matching Sui wallet panel, loads the exact recipient and amount, permits only server refresh of a stale-but-unexpired simulation, rejects an expired handoff without navigation, and preserves exact wallet activity, progressive disclosure, receipt verification state, safe cancellation, all four creation choices, Treasury Coworker creation, exact authority and limits, lifecycle controls, chat binding, reload persistence, and Agent Files continuity.
...
The remaining Phase 1 work is operator-controlled:

- Run, inspect, register, and promote separate sealed Polymarket public-read and wallet-preview certifications. The live acceptance packet must use v2 and prove trusted user-jurisdiction enforcement, direct venue denials, fresh public-book simulation, exact signer/token/FAK terms, mutation and replay denial, connected-wallet-only submission, and receipt reconciliation. The gateway remains off until this evidence passes.
- The restricted MCP Streamable HTTP `2025-11-25`, JSON-RPC 2.0, and OpenAPI action profiles are implemented behind the certified gateway boundary. Each permits only its signed, bounded lifecycle and the closed evidence envelope. The OpenAPI profile signs an origin and one static POST path per action; legacy unprofiled manifests stay non-executable. Live sealed certification and operator promotion remain pending.
