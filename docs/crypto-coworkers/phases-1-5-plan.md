...
- Current verification evidence: 984 application tests and 1,256 server tests pass, including the four-role template, master-prompt budget, tenant-isolation, and wallet-only escalation contracts. The full 10-stage platform safety gate, application and server type checks, shared-types build, secret scan, production web build, and task-first bundle budget also pass. Current rendered desktop and 375 px mobile acceptance confirms all four creation choices, Treasury Coworker creation, exact authority and limits, zero horizontal overflow, and zero unnamed buttons. Earlier rendered acceptance also confirmed lifecycle controls, chat binding, reload persistence, Agent Files continuity, and zero browser console warnings or errors.
