...
- Certified public evidence cache: complete in code. The adapter router can reuse only successful, typed, quarantined, explicitly informational, unauthenticated, scope-free reads that carry a current block or version and remain inside the certified action's freshness window. Cache identity binds the workspace, connection revision and grants, signed manifest and certification hashes, policy version, action, network, projected schema, and canonical argument hash; only the query hash is retained. Every hit still requires a fresh exact-run capability and receives a new usage reconciliation record, while DNS and upstream adapter traffic are skipped and charged at zero tool cost. Private or wallet-linked data, authenticated connections, watch/prepare/simulate actions, unbound observations, expired evidence, paused/revoked connections, and changed certification identities never use the cache. Entries are in-memory, defensively copied, size-bounded, count-bounded, and fail safely back to a live read when admission is unavailable.
