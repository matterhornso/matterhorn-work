...
- Public-evidence receipt identity: complete in code. Every new live or certified-cache result now adds two domain-separated, content-free SHA-256 proofs to the guarded run receipt: one for the exact typed, quarantined model-facing projection and one for the app, revision, action, network, source, observation time, block or version, and projection proof. Live and cached reuse of the same observation retain the same identity while changed evidence, source metadata, app revision, action, or network cannot. The strict reconciliation boundary rejects missing, malformed, open, or forged proof metadata before consuming the reservation. Queries, raw results, wallet identifiers, upstream bodies, capability values, and secrets remain absent; encrypted Walrus evidence binds both proofs through the existing closed evidence hashes. The primary UI stays plain-language, while exact proofs are available only inside technical response details. Earlier receipts without these additive fields remain readable. Current regression evidence is 1,073 application tests, 1,555 server tests, and 36 Crypto App SDK tests with zero failures.
- Public-evidence context continuity: complete in code. Certified adapter results now carry the same exact projection and observation proofs reconciled into their run receipt. Every direct model-facing certified read independently verifies those proofs before returning data. Both the guarded watch executor and the exported watch runner verify them before a model-free guarded run can succeed, establish a baseline, or create an alert, so an alternate executor cannot restore the legacy compatibility path. Coworker watch alerts then bind to the observation proof—which itself binds the projection, app, revision, action, network, source, block/version, and observation time—instead of the older raw-response digest. The exported wallet-intent compiler itself independently recomputes both proofs before compiling a financial result into a wallet review, so a direct or future caller cannot bypass the transaction service's verification boundary. Proof-less legacy results remain display-compatible but cannot create a new scheduled result or wallet intent, while malformed or mutated app, revision, action, network, source, block, observation-time, or typed-result evidence fails closed. Later crypto chats retain only these content-free proof references; legacy source and freshness strings are domain-separated and hashed before reuse, while partial or malformed modern proofs are discarded or rejected rather than entering coworker state. No query, result body, external instruction text, wallet identity, credential, or tenant identity is replayed. Current regression evidence is 1,073 application tests, 1,555 server tests, and 36 Crypto App SDK tests with zero failures, plus the complete 10-stage platform safety gate.
