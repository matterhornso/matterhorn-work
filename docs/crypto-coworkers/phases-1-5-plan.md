...
- Phase 2 account Coworkers control center: complete for the current internal-mode slice. Users can create, select, start a chat with, pause, resume, and revoke Market Analyst and Risk Monitor coworkers through plain-language controls. The panel shows automatic authority, approval boundaries, prohibited authority, limits, watches, alerts, and wallet-review work without exposing signing or submission controls. Starting or selecting a coworker binds its exact identity and revision to the session, and that context survives reloads until the user clears it.
...
- Current verification evidence: 979 application tests, 1,229 server tests, the full 10-stage platform safety gate, application and server type checks, shared-types build, secret scan, production web build, and task-first bundle budget pass. Rendered desktop and 375 px mobile acceptance confirmed Coworker creation, lifecycle controls, chat binding, reload persistence, Agent Files continuity, zero horizontal overflow, and zero browser console warnings or errors.
