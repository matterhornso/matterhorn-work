...
- Phase 1 Sui SDK boundary: complete and backend-only. Matterhorn injects an explicit binary gRPC-web transport over a TLS-verified, IPv4-pinned HTTP/2 socket. The exact allowlist contains only `StateService/GetBalance`, `StateService/GetCoinInfo`, `LedgerService/GetServiceInfo`, `LedgerService/BatchGetObjects`, and `TransactionExecutionService/SimulateTransaction` for typed balance, coin-metadata, checkpoint-freshness, Walrus object certification, and simulation work. `BatchGetObjects` is reached only by the official Walrus object parser and remains a read surface. `ExecuteTransaction`, `GetTransaction`, and every other method fail before dialing.
