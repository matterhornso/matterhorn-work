...
- Phase 1 schema admission hardening: complete. The shared SDK/server evaluator requires bounded ASCII property names and rejects secret, credential, signature, signing-payload, transaction-byte, and sign/submit/relay/broadcast authority fields at every input/output nesting depth. Unsafe manifests cannot produce SDK signing bytes, invoke the local adapter runner, pass server conformance, or reach typed model-facing projection.
