...
- Phase 1 trusted Polymarket jurisdiction boundary: complete in code and dormant. The Vercel same-origin function removes caller-supplied Matterhorn and Vercel geo headers before forwarding, then may create one 60-second HMAC proof from Vercel's country, region, and anti-spoofed client-IP signals. The proof is bound to the exact method, normalized backend route, hashed peer, and hashed Vercel request identity. The backend accepts it only with the independent trusted-proxy secret and rejects changed, stale, future, open, malformed, wrong-route, wrong-method, wrong-peer, or direct-backend claims. A closed, server-owned policy reproduces the official Polymarket country and subregion restrictions, denies close-only and frontend-restricted locations from opening positions, requires a region where the official policy is region-specific, and fails closed after its explicit 2026-10-04 review deadline. Its content-free decision is bound to the exact evidence, guarded run, and single-use capability and expires no later than the 60-second edge proof. Country, region, IP, and Vercel request ID never enter provider messages or security receipts. Missing or invalid proof does not impede public research but grants no preparation authority. Because Polymarket's browser geoblock response is unsigned, it can only add a denial and can never override the server policy. Certified order preparation remains disabled until a wallet-only simulation adapter and exact reviewed intent are separately proven.
