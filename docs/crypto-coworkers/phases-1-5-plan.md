...
- Phase 4 Agent Files cloud boundary: complete at the backend testnet boundary. A collaborator must explicitly confirm each exact file revision before Matterhorn uploads the ciphertext-only envelope. Matterhorn verifies pinned Sui certification and byte-exact Walrus readback before storing redacted publication metadata; tenant identifiers, filenames, coworker IDs, KMS references, and plaintext never enter public bytes. Duplicate concurrent publication is serialized, account isolation is enforced, deletion destroys the recovery key, and mainnet remains disabled. Read-only verification now reports healthy and renewal-due storage windows and fails closed after expiry; agents receive no renewal, signing, or payment authority.
...
- Current Phase 4 slice: authenticated original-file recovery is complete with tenant, revision, expiry, and integrity checks plus a no-store browser download; wallet/infrastructure-controlled renewal execution remains pending. Expiry monitoring and ciphertext-only Quilt batching are implemented, but renewal is deliberately not automatic. Automatic finalized-run sealing is local-only and never publishes; testnet publication stays server-owned and configuration-gated; mainnet remains disabled.
