...
- Phase 1 trusted user-jurisdiction prerequisite: complete in code and dormant. The Vercel same-origin function removes caller-supplied Matterhorn and Vercel geo headers before forwarding, then may create one 60-second HMAC proof from Vercel's country and anti-spoofed client-IP signals. The proof is bound to the exact method, normalized backend route, hashed peer, and hashed Vercel request identity. The backend accepts it only with the independent trusted-proxy secret and rejects changed, stale, future, open, malformed, wrong-route, wrong-method, wrong-peer, or direct-backend claims. A stable content-free country-and-peer digest binds privacy consent, guarded-run state, and issued capabilities; raw IP, Vercel request ID, and country are never added to provider messages or receipts. Missing proof does not impede public research but grants no compliance authority. This is not a Polymarket eligibility decision and does not enable order preparation.
