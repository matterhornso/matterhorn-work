...
- Phase 1 Polymarket public-read boundaries: complete in code and backend-only for active-market discovery plus exact-token order-book evidence. Gamma discovery and CLOB reads use separate signed manifests, separate pinned HTTPS origins, bodyless GETs, canonical uint256 token IDs, closed typed projections, bounded sorted levels, and read-only coworker capabilities. Account, compliance, order, cancellation, signing, relay, submission, credentials, and destination overrides are excluded. Registration and sealed operator certification remain pending; the gateway stays off by default.
