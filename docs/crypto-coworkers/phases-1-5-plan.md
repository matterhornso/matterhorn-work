...
- Current Phase 4 slice: authenticated original-file recovery is complete with tenant, revision, expiry, and integrity checks plus a no-store browser download. Testnet renewal is implemented as a connected-wallet airlock: the server prepares and simulates exact bytes, the browser verifies the digest, the wallet reviews/signs/submits, and the server records the new certification only after exact public-chain reconciliation. Renewal is never automatic and cannot be invoked by an agent. Expiry monitoring and ciphertext-only Quilt batching are implemented. Automatic finalized-run sealing is local-only and never publishes; initial testnet publication stays server-owned and configuration-gated; mainnet remains disabled.
- Current verification evidence: 991 application tests and 1,256 server tests pass, including the four-role template, master-prompt budget, tenant isolation, wallet-only escalation, cross-account wallet-intent cancellation and receipt denial, exact wallet-review lifecycle contracts, one-shot handoff binding, mutation rejection, and stale/expired behavior. The full 10-stage platform safety gate, application and server type checks, shared-types build, strict secret scan, production web build, task-first bundle budget, and Impeccable detector also pass. Current rendered 1440 px desktop and 375 px mobile acceptance confirms that the desktop workspace rail remains visible while a compact mobile menu exposes Coworkers, Files, MCPs, Memory, Notes, Wallet, Profile, and every crypto desk; Coworkers and Sui restore through shareable panel routes; the menu has no unnamed controls or horizontal overflow; and the browser emits no console errors. Earlier rendered acceptance confirmed that a coworker row opens the matching Sui wallet panel, loads the exact recipient and amount, permits only server refresh of a stale-but-unexpired simulation, rejects an expired handoff without navigation, and preserves exact wallet activity, progressive disclosure, receipt verification state, safe cancellation, all four creation choices, Treasury Coworker creation, exact authority and limits, lifecycle controls, chat binding, reload persistence, and Agent Files continuity.
...
The first-party testnet contracts and backend-only executors cover Sui balance reads and dry-run transfer preparation plus Hyperliquid market, orderbook, account-exposure, and order-preparation actions. Every testnet contract is signed in the test harness, closed-schema projected, wallet-submission-only, and bound to a compatible existing guarded tool. A separate signed Polymarket mainnet public-research contract now covers one bounded Gamma `/public-search` read. Matterhorn—not the model or upstream response—constructs its exact bodyless same-origin GET and projects only typed market metadata. The contract excludes profiles, arbitrary destinations or methods, CLOB, geoblock, wallet, preparation, signing, relay, and submission authority, and the testnet promotion command rejects it. Test fixtures prove unknown/private and instruction-like payload fields do not enter model-facing results. Live testnet egress and the Polymarket request boundary are pinned and method-bounded. Registration and certification promotion remain trusted-operator actions; account users receive only redacted discovery, connection, coworker, and wallet-review surfaces.

The next Phase 1 slice must:

- Define a reviewed Bittensor public-read boundary over the existing Subtensor/sidecar architecture without inventing an unstable REST contract or allowing destination overrides. The bounded Polymarket Gamma discovery read is complete; multi-origin Polymarket compliance, orderbook, and preparation still require separately certified boundaries.
