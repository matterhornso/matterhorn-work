...
- Compose the trusted user-jurisdiction proof with the venue's direct browser eligibility decision without treating either the backend egress location or client-authored JSON as authority. Order preparation remains out of scope until that composition and a wallet-only simulation contract are proven.
