...
- The restricted MCP Streamable HTTP `2025-11-25` transport profile is implemented behind the certified gateway boundary. It performs only initialize, initialized notification, and one exact signed `tools/call`; requires a closed `structuredContent` evidence envelope; and excludes discovery, SSE, prompts, resources, sampling, elicitation, tasks, destination overrides, arbitrary methods, and upstream cost claims. The restricted JSON-RPC 2.0 profile performs one exact signed method call with request-ID binding and the same closed evidence boundary; batch calls, notifications, discovery, subscriptions, callbacks, and upstream cost claims are excluded. Live sealed certification and operator promotion remain pending. OpenAPI now fails closed because manifest v1 cannot sign an exact operation path and method; a versioned operation-binding contract is required before certification.
