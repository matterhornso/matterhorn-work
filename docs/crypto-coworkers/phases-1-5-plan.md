...
- Current Phase 4 slice: wallet/infrastructure-controlled renewal execution and simplified recovery controls remain pending. Expiry monitoring and ciphertext-only Quilt batching are implemented, but renewal is deliberately not automatic. Automatic finalized-run sealing is local-only and never publishes; testnet publication stays server-owned and configuration-gated; mainnet remains disabled.
