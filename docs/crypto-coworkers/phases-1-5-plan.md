...
- Current slice: execute the sealed runtime harness against the pinned Sui and Hyperliquid testnet adapters using operator-controlled testnet identities, then promote the resulting reports. Publisher signing keys and testnet accounts remain outside source and server configuration.
