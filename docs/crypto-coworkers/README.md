...
Phase 2 profile storage is available only when `MATTERHORN_COWORKER_MODE` is not `off`, an explicit `MATTERHORN_COWORKER_POLICY_VERSION` is configured, and `MATTERHORN_COWORKER_INTEGRITY_SECRET` contains an independent server-only value of at least 32 bytes. It defaults to `<data-dir>/crypto-coworkers/coworkers.db` and may be overridden with `MATTERHORN_COWORKER_DB`. Invite hashes and account-access rows are authenticated by domain-separated AES-256-GCM authority seals; fabricated access, invite extension or reuse, tenant reassignment, invalid state transitions, and wrong-key restoration fail closed. Authority-bearing reads reverify the exact row, and a one-time migration seals only structurally valid legacy admission state. Rotating the integrity secret requires an explicit access re-enrollment migration. Off mode performs no database access or integrity-secret validation.
