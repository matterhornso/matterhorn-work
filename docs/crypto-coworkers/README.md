...
Account users can inspect their redacted evidence packets through `GET /workspace/:id/crypto-evidence`, explicitly store one exact sealed revision through `POST /workspace/:id/crypto-evidence/:evidenceId/publish`, request independent read-only verification through `POST /workspace/:id/crypto-evidence/:evidenceId/verify`, and irreversibly delete one exact revision's recovery key through `DELETE /workspace/:id/crypto-evidence/:evidenceId/recovery-key`. Publication and key deletion each require an explicit owner confirmation and are mutually serialized through a short-lived durable SQLite operation claim. Published run evidence is also checked at startup and every six hours by a bounded read-only verifier; its redacted, exact-revision status is shown in the account response and discarded as soon as the evidence revision changes. The deletion response states whether residual public ciphertext may remain and never claims that a public Walrus copy was erased. The server derives the owner from the authenticated workspace, never accepts an owner override, and publishes only the generic encrypted envelope; tenant identifiers, coworker IDs, KMS references, wrapped keys, prompts, signatures, and wallet data remain private. Testnet publication and verification recompute the ciphertext hash and Merkle proof, require pinned Sui certification, and compare an independently retrieved Walrus blob byte-for-byte before storing or accepting proof metadata. None of these routes signs, anchors, pays, renews, or submits a transaction. When evidence mode is `off`, the list returns an explicit unavailable state and publication and live verification fail closed; local owner-controlled key deletion remains available when the evidence KMS boundary is configured.
...
- Canonical `crypto-intent.v1` production from certified Sui, Hyperliquid, and Bittensor adapter outputs is complete. The compiler cross-checks request and result terms, rejects stale simulations, hash-binds exact terms and policy, and regenerates only wallet-review-only v2 handoffs.
...
- Expiry warning, fail-closed verification, user deletion, encryption-key destruction, and connected-wallet testnet renewal are complete for Agent Files. Renewal uses an exact simulated transaction and single-use server confirmation; automatic, agent-initiated, infrastructure-submitted, and mainnet renewal remain disabled.

The implemented foundation can perform an authenticated testnet Walrus ciphertext upload, independent pinned Sui certification reads, and connected-wallet Agent File storage renewal. The account-facing surface lists redacted records, reports local/key-destroyed states, asks for an explicit acknowledgement before publishing one exact encrypted run-evidence revision, automatically refreshes read-only integrity health, verifies an existing publication on demand without disclosing ciphertext or tenant metadata, and opens an exact Agent File renewal transaction in the user's wallet only when storage is nearly due. Finalized coworker runs are sealed automatically into local encrypted evidence; neither run evidence nor Agent Files are published automatically. Ciphertext-only Quilt batching is implemented. Automatic publication is intentionally disabled; production anchor creation remains pending. Matterhorn itself performs no Sui transaction submission, and mainnet remains fail-closed and disabled.
