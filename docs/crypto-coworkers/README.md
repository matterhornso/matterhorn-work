...
Account users can inspect their redacted evidence packets through `GET /workspace/:id/crypto-evidence`, explicitly store one exact sealed revision through `POST /workspace/:id/crypto-evidence/:evidenceId/publish`, and request independent read-only verification through `POST /workspace/:id/crypto-evidence/:evidenceId/verify`. Publication requires an owner-confirmed public-ciphertext acknowledgement, is serialized through a short-lived durable SQLite claim, and is testnet-only. The server derives the owner from the authenticated workspace, never accepts an owner override, and publishes only the generic encrypted envelope; tenant identifiers, coworker IDs, KMS references, wrapped keys, prompts, signatures, and wallet data remain private. Testnet publication and verification recompute the ciphertext hash and Merkle proof, require pinned Sui certification, and compare an independently retrieved Walrus blob byte-for-byte before storing or accepting proof metadata. Neither route signs, anchors, pays, renews, or submits a transaction. When evidence mode is `off`, the list returns an explicit unavailable state and publication and live verification fail closed.
