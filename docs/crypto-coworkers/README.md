...
The hosted same-origin proxy now has a separate trusted-jurisdiction prerequisite for future Polymarket preparation. It strips every caller-supplied internal and Vercel geo header, hashes rather than forwards the raw Vercel request identity, and creates a 60-second HMAC proof bound to Vercel's country/client-IP signal, the exact method, and the normalized backend route. The backend accepts it only with the existing trusted-proxy secret. Its stable content-free digest is bound into exact-request privacy consent, guarded-run state, and capabilities, but country and IP never enter the provider prompt or security receipt. Missing or invalid proof grants no compliance authority. This proof is deliberately not treated as venue eligibility: the direct browser geoblock check and a wallet-only order simulation still need a fail-closed composition before certified Polymarket preparation can be enabled.
