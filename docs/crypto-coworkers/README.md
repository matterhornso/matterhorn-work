...
The first-party executor can now read Sui balance/checkpoint data and Hyperliquid market, orderbook, and account data over the same DNS-pinned, TLS-verified requester used by the gateway. Hyperliquid order preparation refreshes testnet market definitions, book, margin state, lot precision, leverage, and slippage before producing a short-lived hash-bound wallet-review reference; it never calls the exchange endpoint. Sui transfer preparation deliberately fails before network access because the current exact transaction builder/dry-run uses a separate gRPC transport that has not yet been brought inside the pinned egress boundary. These executors remain disconnected from server startup and all production modes remain off.
