...
MATTERHORN_CRYPTO_APP_CONNECTION_INTEGRITY_SECRET=<independent-server-only-secret>
MATTERHORN_CRYPTO_APP_DEVELOPER_INTEGRITY_SECRET=<independent-server-only-secret>
MATTERHORN_CRYPTO_APP_OPERATIONAL_INTEGRITY_SECRET=<independent-server-only-secret>
...
The operational-policy store defaults to `<data-dir>/crypto-apps/operational.db` and may be overridden with `MATTERHORN_CRYPTO_APP_OPERATIONAL_DB`. It atomically reserves a bounded per-call allowance against a workspace/day ceiling, reconciles measured cost exactly once, expires abandoned reservations without allowing call replay, and persists tenant-scoped circuit state across restarts. Every restored reservation and circuit row is authenticated with a domain-separated AES-256-GCM authority seal derived from the independent server-only `MATTERHORN_CRYPTO_APP_OPERATIONAL_INTEGRITY_SECRET`; silent cost, tenant, replay, outcome, expiry, or circuit-state changes fail startup closed, and selected rows are reverified before authority-bearing updates. A one-time schema migration validates and seals structurally valid legacy rows only when the seal column is first introduced. Its rows contain only identifiers, counters, timestamps, and outcomes—never prompts, arguments, credentials, wallet data, or adapter output. The default backend-only policy reserves at most 1,000,000 micro-units per call and 10,000,000 per workspace UTC day; launch-specific values must be explicitly configured when the gateway is wired into startup.
...
The hosted same-origin proxy now provides the trusted jurisdiction half of future Polymarket preparation. It strips every caller-supplied internal and Vercel geo header, hashes rather than forwards the raw Vercel request identity, and creates a 60-second HMAC proof bound to Vercel's country, region, anti-spoofed client-IP signal, exact method, and normalized backend route. The backend accepts it only with the existing trusted-proxy secret. A closed, server-owned and review-dated policy applies the official Polymarket country and subregion restrictions, denies new positions from blocked, close-only, and frontend-restricted locations, and fails closed when required regional evidence is absent or the policy reaches its review deadline. The resulting content-free decision is bound into the guarded run and single-use capability and expires with the edge proof; country, region, IP, and request ID never enter the provider prompt or security receipt. Missing or invalid proof grants no preparation authority but does not impede public research. Polymarket's unsigned direct-browser geoblock response remains an additional fail-closed denial immediately before wallet work; it can never grant authority or override the server policy. A certified wallet-only order simulation and exact reviewed intent still must be proven before Polymarket preparation is enabled.
...
Phase 2 profile storage is available only when `MATTERHORN_COWORKER_MODE` is not `off`, an explicit `MATTERHORN_COWORKER_POLICY_VERSION` is configured, and `MATTERHORN_COWORKER_INTEGRITY_SECRET` contains an independent server-only value of at least 32 bytes. It defaults to `<data-dir>/crypto-coworkers/coworkers.db` and may be overridden with `MATTERHORN_COWORKER_DB`. Invite hashes, account-access rows, profiles, compact working state, exact resource scopes, chat bindings, watch schedules, and inbox items are authenticated by distinct domain-separated AES-256-GCM authority seals. Fabricated access, invite extension or reuse, tenant or agent reassignment, policy or resource broadening, chat rebinding, schedule/evidence mutation, invalid lifecycle state, and wrong-key restoration fail closed before the restored state can become runtime authority or account-visible evidence. Authority-bearing reads and updates reverify the exact row and rotate its seal, while bounded inbox pruning deletes only authenticated rows by exact seal. A one-time migration seals only structurally valid, relationship-valid, secret-free legacy admission and execution state. Rotating the integrity secret requires an explicit access and coworker-state re-enrollment migration. Off mode performs no database access or integrity-secret validation.
...
Every live encrypted Evidence and Agent File record is also admitted through a server-only durable-state authority before it can be listed, decrypted, published, renewed, anchored, recovered, or deleted. A domain-separated HMAC derived with HKDF from `MATTERHORN_CAPABILITY_SIGNING_SECRET` binds the complete record to its state kind, storage key, workspace, session, expiry, and SQLite update time. Tenant transplantation, revision or publication mutation, recovery-key substitution, stale-value replay onto an advanced row, a wrong authority key, and unsealed legacy state all fail closed. The authority seal deliberately does not claim to distinguish an exact byte-for-byte restoration of an older signed row from its original state; the independent recovery-erasure ledger below prevents such a restore from reviving destructively erased recovery material.

The same authority now protects every short-lived connected-wallet review intent for Evidence and Agent File Walrus renewal, Evidence Walrus deletion, and immutable Sui anchoring. Raw legacy rows, modified payloads or expiry, tenant transplantation, and wrong-key restoration fail closed before wallet receipt or chain verification. Consuming a review removes it atomically; restoring an older correctly signed intent cannot regain authority after the separately authenticated Evidence or Agent File operation claim and revision have advanced.

Every exclusive Evidence and Agent File operation claim is authenticated as well. A publication, renewal, deletion, immutable anchor, key rotation, or key destruction can start, remain current, and release its lock only through an exact sealed claim bound to its storage key, tenant, expiry, update time, operation, record revision, and random claim ID. Fabricated legacy claims, metadata or payload mutation, tenant transplantation, and wrong-key restoration fail closed before public storage, chain verification, or recovery-key work can use the claim. A stale worker cannot release a later replacement claim.
...
- Pending wallet reviews are authenticated at rest with a domain-separated HMAC-SHA256 envelope derived from `MATTERHORN_COWORKER_INTEGRITY_SECRET`. The envelope binds the exact tenant, owner, coworker, session, storage key, intent, policy, reviewed-action v2, receipt, revision, expiry, and update time. A restored review with a changed owner, wallet terms, lifecycle state, SQL-row metadata, or seal fails closed. Old unsealed reviews are display-incompatible and must be regenerated before wallet use; Matterhorn does not migrate unauthenticated review state into transaction authority.
