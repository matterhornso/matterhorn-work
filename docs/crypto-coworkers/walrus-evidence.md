...
- Agent File publication and recovery-key destruction use one tenant- and revision-bound five-minute SQLite operation claim before publisher or key-manager work. The claim survives a server restart, blocks upload/deletion races and duplicate publisher spend in either direction, expires for recovery, and can be released only by the worker that owns its random claim ID. An expired worker cannot clear a newer worker's claim.
