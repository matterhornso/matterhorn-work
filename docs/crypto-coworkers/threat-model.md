...
| JSON-RPC peer broadens protocol authority | One signed method, fresh response-bound ID, closed evidence result, and no batches, notifications, discovery, subscriptions, callbacks, or upstream cost claims | Method substitution, peer requests, response replay, extra fields, and injected error detail fail closed |
| Mutable OpenAPI operation document | Ignore dynamic documents; signed profile binds an origin and one static POST path per action; legacy manifests cannot certify or execute | Changed descriptions, server lists, paths, methods, callbacks, links, queries, and remote references cannot redirect an approved action |
