...
| Cross-vertical workflow catalog | `GET /api/workflows/catalog` returns `matterhorn.workflow.catalog.v1`; `GET /api/workflows/prompts` returns `matterhorn.workflow.prompt-pack.v1` for copy-pasteable staged prompts; no provider execution | `matterhorn_workflows_catalog` and `matterhorn_workflows_prompt_pack` expose the same catalog/prompt-pack surfaces for Claude Code, Codex, Cursor, Claude Desktop, and other MCP clients | `matterhorn-work workflows catalog --json`; `matterhorn-work workflows prompts --workflow wellness_creator_workflow --json`; optional `--workflow <id>`, `--category <name>`, `--include-prompts` | `test:matterhorn-workflow-catalog`, `test:matterhorn-workflow-catalog-api-mcp`, `test:matterhorn-workflow-contract`, `test:agent-control-mcp`, `test:market-execution-safety-gate` |
