...
| Unified crypto chat router | `POST /api/crypto/chat/execute` with versioned `matterhorn.crypto.shared-card.v1` `sharedCards` for readiness report, discovery, account snapshot, market/orderbook context, preview, compliance block, signer handoff, receipt/status, and watch alert | `matterhorn_crypto_chat`; venue MCP tools remain available after routing: `matterhorn_bittensor_chat`, `matterhorn_hyperliquid_chat`, `matterhorn_polymarket_chat` | `matterhorn-work crypto chat` (aliases `market`/`markets chat`); venue CLI fallbacks: `matterhorn-work bittensor chat`, `matterhorn-work hyperliquid chat`, `matterhorn-work polymarket chat` | `test:unified-crypto-chat`, `test:crypto-direct-prompt-safety`, `test:unified-crypto-shared-card-contract`, `test:crypto-cli-fallback`, `test:agent-crypto-operator-loop`, `test:market-execution-safety-gate` |
| Customer crypto readiness | `GET /api/crypto/readiness` summarizes Bittensor, connected-wallet Hyperliquid execution, and Polymarket read/preview boundaries; offline smoke and customer packet evidence remain required before a final customer handoff | `matterhorn_crypto_readiness` reads the same server report but cannot submit; use venue tools for read/preview detail | `matterhorn-work crypto readiness`; CLI cannot submit orders | `test:crypto-readiness-api`, `test:agent-control-mcp`, `test:crypto-cli-fallback`, `test:customer-ready-crypto-smoke`, `test:market-execution-safety-gate` |
| Bittensor beta release gate | No server route; offline launch gate for a Bittensor-first customer beta while Hyperliquid/Polymarket remain preview/R&D-only | No MCP wrapper; agents should run the CLI/script and attach JSON output to the customer handoff | `pnpm smoke:bittensor-beta`; raw helper is `node scripts/bittensor-beta-release-gate.mjs --offline --strict --json-output <path>` | `test:bittensor-beta-release-gate`, `test:bittensor-customer-readiness-gate`, `test:market-execution-safety-gate`, `test:customer-ready-crypto-smoke` |
| Bittensor beta release candidate packet | No server route; offline go-live packet that indexes beta-gate, customer-smoke, Bittensor evidence, live public QA, browser QA, rollback, and hashes | No MCP wrapper; agents should attach the generated JSON/Markdown/SHA files to the customer handoff | `pnpm beta:bittensor:packet` for fixture shape; real RC uses `node scripts/bittensor-beta-customer-packet.mjs --beta-gate <json> --customer-ready-smoke <json> --bittensor-evidence-verify <json> --browser-qa <md> --strict` | `test:bittensor-beta-customer-packet`, `test:bittensor-beta-release-gate`, `test:customer-ready-crypto-smoke` |
| Live public-data QA pack | No server route; aggregates existing live read-only routes and offline evidence into public/redacted customer artifacts | `matterhorn_crypto_live_public_qa` runs the same local QA pack in fixture mode by default, uses MCP environment auth for optional live server checks, and returns JSON/Markdown/SHA evidence paths | `matterhorn-work crypto live-public-qa --output-dir <dir>` writes JSON, Markdown, and SHA-256 evidence; `--fixture` produces `SKIPPED_WITH_FIXTURE_FALLBACK` without live inputs | `test:crypto-live-public-qa`, `test:agent-control-mcp`, `test:crypto-cli-fallback`, `test:market-execution-safety-gate`, `test:customer-ready-crypto-smoke` |
...
| Hyperliquid read, preview, and web execution | Read/preview routes plus guarded `POST /api/hyperliquid/orders/execution-intent` and `POST /api/hyperliquid/orders/submit`; submit requires a matching connected-wallet signature and deployment kill switch | MCP remains read/preview/handoff only and has no submit tool | CLI remains read/preview/handoff only and has no submit command | `hyperliquid-live-execution.test.ts`, `test:hyperliquid-read-preview-qa`, `test:hyperliquid-cli-fallback`, `test:market-execution-safety-gate` |
...
| Market watch alerts | `GET/POST /api/hyperliquid/watches`, `POST /api/hyperliquid/watches/check`, `GET /api/hyperliquid/watches/digest`, `POST /api/hyperliquid/watches/act`, plus matching Polymarket watch routes | `matterhorn_hyperliquid_create_watch`, `matterhorn_hyperliquid_check_watches`, `matterhorn_hyperliquid_watch_digest`, `matterhorn_hyperliquid_act_on_watch_alert`, `matterhorn_polymarket_create_watch`, `matterhorn_polymarket_check_watches`, `matterhorn_polymarket_watch_digest`, `matterhorn_polymarket_act_on_watch_alert` | `matterhorn-work hyperliquid watch create/list/check/digest/act`, `matterhorn-work polymarket watch create/list/check/digest/act` | `test:market-watch-workflows`, `test:market-watch-action-routes`, `test:unified-crypto-chat`, `test:market-execution-safety-gate` |
...
| Market official SDK evidence | `GET /api/crypto/market-sdk-validation` returns the read-only `matterhorn.market.sdk-validation-guide.v1` for fixture/operator-owned testnet validation; artifact validation remains offline/public-redacted and accepts no secrets | `matterhorn_market_sdk_validation` prints the same local official SDK validation guide; `matterhorn_market_customer_evidence_verify` verifies already-loaded public/redacted customer evidence bundle JSON/Markdown and does not read files, sign, submit, or accept secrets | `matterhorn-work crypto customer-smoke` for the consolidated smoke report; `matterhorn-work crypto sdk-doctor` for validation environment readiness; `matterhorn-work crypto sdk-normalize` for redacted official-client artifacts; `matterhorn-work crypto sdk-capture` for normalized public evidence capture; `matterhorn-work crypto sdk-evidence` for offline validation/sample evidence JSON; `matterhorn-work crypto sdk-loop` for the copy-pasteable SDK loop plus `matterhorn-market-sdk-run-manifest.json`; `matterhorn-work crypto sdk-manifest-check` for offline manifest hash/safety validation; `matterhorn-work crypto evidence-bundle` for the final customer bundle, optionally with public receipt-check evidence; `matterhorn-work crypto evidence-verify` for final offline bundle verification; raw helpers remain available: `node scripts/market-official-sdk-validation-doctor.mjs`, `node scripts/market-official-sdk-normalize.mjs`, `node scripts/market-official-sdk-validation-capture.mjs`, `node scripts/market-official-sdk-operator-loop.mjs`, `node scripts/market-sdk-run-manifest-check.mjs`, `node scripts/market-customer-evidence-verify.mjs` | `test:agent-control-mcp`, `test:customer-ready-crypto-smoke`, `test:market-official-sdk-validation-doctor`, `test:market-official-sdk-normalize`, `test:market-official-sdk-operator-loop`, `test:market-sdk-run-manifest-check`, `test:market-official-sdk-validation-capture`, `test:market-official-sdk-validation-track`, `test:market-customer-evidence-bundle`, `test:market-customer-evidence-verify`, `test:crypto-cli-fallback` |
| Market execution-readiness gate | `GET /api/crypto/market-execution-readiness` reports whether connected-wallet Hyperliquid execution is enabled and keeps Polymarket submission disabled | `matterhorn_market_execution_readiness` reads readiness but cannot submit | `matterhorn-work crypto execution-readiness` reads readiness but cannot submit; security docs live in `docs/market-execution-readiness-security-gate.md` | `test:market-execution-readiness-api`, `test:market-execution-readiness-gate`, `test:market-execution-safety-gate`, `test:customer-ready-crypto-smoke` |
| Legacy market submit/sign contract | The generic Phase 0 operator contract remains no-submit. Hyperliquid web execution uses a narrower exact-intent route and does not activate the generic signed-submission envelope | No live execution MCP | No live execution CLI | `market.submit_sign_phase0_contract`, `test:market-submit-sign-contract-phase0`, `test:market-execution-safety-gate` |
| Market sign-request Phase 1 | `POST /api/hyperliquid/orders/external-sign-request` and `POST /api/polymarket/orders/external-sign-request` create `matterhorn.market.external-sign-request.v1` packets only when `executionMode=testnet_external_signer`; no signed artifact intake or submit route | `matterhorn_hyperliquid_create_sign_request`, `matterhorn_polymarket_create_sign_request`; both require explicit testnet mode and reject credential-shaped inputs | `matterhorn-work hyperliquid sign-request --execution-mode testnet_external_signer`, `matterhorn-work polymarket sign-request --execution-mode testnet_external_signer`; outputs remain `canSubmit:false` and `submitSignedAllowedByContract:false` | `market.sign_request_phase1`, `test:market-sign-request-phase1`, `test:market-sign-artifact-routes`, `test:market-submit-sign-contract-phase0`, `test:market-execution-safety-gate`, `test:customer-ready-crypto-smoke` |
| Market artifact validation Phase 2 | `POST /api/hyperliquid/orders/external-artifact/validate` and `POST /api/polymarket/orders/external-artifact/validate` validate `matterhorn.market.redacted-signed-artifact-envelope.v1` metadata against a Phase 1 sign request and return `matterhorn.market.artifact-validation.v1` plus a public receipt candidate | `matterhorn_hyperliquid_validate_external_artifact`, `matterhorn_polymarket_validate_external_artifact`; both accept public/redacted metadata only and reject raw artifact material | `matterhorn-work hyperliquid validate-artifact --sign-request-file <path> --artifact-file <path>`, `matterhorn-work polymarket validate-artifact --sign-request-file <path> --artifact-file <path>`; still no submit path | `market.artifact_validation_phase2`, `test:market-artifact-validation-phase2`, `test:market-sign-artifact-routes`, `test:market-sign-request-phase1`, `test:market-execution-safety-gate`, `test:customer-ready-crypto-smoke` |
| Market execution chain gate | `GET /api/crypto/market-execution-chain` returns the same no-submit `matterhorn.market.execution-chain-guide.v1`; aggregate contract covers preview/handoff, explicit testnet external sign-request, public/redacted artifact validation, artifact reconciliation, and public receipt import; every step remains hash-bound and customer-safe | `matterhorn_market_execution_chain` prints the same local safe command plan; MCP coverage also spans `matterhorn_hyperliquid_create_sign_request`, `matterhorn_polymarket_create_sign_request`, `matterhorn_hyperliquid_validate_external_artifact`, `matterhorn_polymarket_validate_external_artifact`, `matterhorn_market_artifact_reconcile`, and venue receipt tools | `matterhorn-work crypto execution-chain` prints the local safe command plan; Demo tab and CLI expose copy-only venue commands: `matterhorn-work hyperliquid sign-request`, `matterhorn-work polymarket sign-request`, `matterhorn-work hyperliquid validate-artifact`, `matterhorn-work polymarket validate-artifact`, and venue receipt commands; no submit command exists | `market.execution_chain_gate`, `test:market-execution-chain-gate`, `test:agent-control-mcp`, `test:crypto-cli-fallback`, `test:customer-readiness-ui`, `test:market-sign-request-phase1`, `test:market-artifact-validation-phase2`, `test:market-artifact-reconciliation`, `test:market-receipt-qa`, `test:market-execution-safety-gate` |
| Market artifact reconciliation | No server route; offline customer evidence checker over one or more `matterhorn.market.artifact-validation.v1` outputs that verifies accepted public metadata, public audit receipt candidates, and no submit/sign capability | `matterhorn_market_artifact_reconcile` accepts already-loaded public/redacted validation objects and returns `matterhorn.market.artifact-reconciliation.v1`; `matterhorn_market_customer_evidence_verify` can require attached reconciliation evidence | `matterhorn-work crypto artifact-reconcile --hyperliquid-artifact-validation <path> --polymarket-artifact-validation <path>` writes `matterhorn.market.artifact-reconciliation.v1`; raw helper remains `node scripts/market-artifact-reconciliation.mjs`; `matterhorn-work crypto evidence-bundle --artifact-reconciliation ... --require-artifact-reconciliation` attaches it to final market evidence | `market.artifact_reconciliation`, `test:agent-control-mcp`, `test:market-artifact-reconciliation`, `test:crypto-cli-fallback`, `test:market-artifact-validation-phase2`, `test:market-customer-evidence-bundle`, `test:market-execution-safety-gate`, `test:customer-ready-crypto-smoke` |
...
| Bittensor external signing | `POST /api/bittensor/extrinsics/prepare`, `POST /api/bittensor/extrinsics/handoff`, `POST /api/bittensor/extrinsics/receipt`; server submission remains outside agent/MCP/CLI authority | `matterhorn_bittensor_prepare_extrinsic`, `matterhorn_bittensor_create_signing_handoff`, `matterhorn_bittensor_import_receipt`; the deprecated submit name is hidden and fails with `wallet_airlock_required` | `matterhorn-work bittensor extrinsic prepare/handoff`; the wallet panel exposes a `Copy Import` command for public receipt evidence; deprecated submit fails closed | `test:agent-control-mcp`, `test:bittensor-cli-fallback`, Bittensor server tests, crypto authority gate |
...
| Cross-vertical workflow catalog | `GET /api/workflows/catalog` returns `matterhorn.workflow.catalog.v1`; `GET /api/workflows/prompts` returns `matterhorn.workflow.prompt-pack.v1`; `GET /api/workflows/templates` returns `matterhorn.customer.workflow.template.v1` for app entrypoints and agent launchers; no provider execution | `matterhorn_workflows_catalog`, `matterhorn_workflows_prompt_pack`, and `matterhorn_workflows_customer_templates` expose the same catalog/prompt/template surfaces for Claude Code, Codex, Cursor, Claude Desktop, and other MCP clients | `matterhorn-work workflows catalog --json`; `matterhorn-work workflows prompts --workflow wellness_creator_workflow --json`; `matterhorn-work workflows templates --customer-template bittensor_operator --json`; optional `--workflow <id>`, `--customer-template <id>`, `--category <name>`, `--include-prompts` | `test:matterhorn-workflow-catalog`, `test:matterhorn-workflow-template-registry`, `test:matterhorn-workflow-catalog-api-mcp`, `test:matterhorn-workflow-contract`, `test:agent-control-mcp`, `test:market-execution-safety-gate` |
