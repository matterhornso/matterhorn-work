...
| Customer crypto readiness | `GET /api/crypto/readiness` summarizes Bittensor runtime readiness plus Hyperliquid/Polymarket read-preview safety surfaces; offline smoke and customer packet evidence remain required before a final customer handoff | Not yet exposed as MCP; use `matterhorn_crypto_chat` for customer-facing crypto questions and venue tools for detail | No dedicated CLI route yet; run `pnpm smoke:customer-ready-crypto` and `matterhorn-work crypto customer-packet` for evidence-backed customer readiness | `test:crypto-readiness-api`, `test:customer-ready-crypto-smoke`, `test:market-execution-safety-gate` |
