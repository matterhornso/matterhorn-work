...
| Retired market sign-request compatibility | The former Hyperliquid and Polymarket external-sign-request routes return `409 wallet_airlock_required` before reading the body or making provider traffic | Former MCP names are hidden, local fail-closed compatibility stubs and are absent from `tools/list` | Legacy CLI calls receive `wallet_airlock_required`; they are not part of the supported transaction path | `market.sign_request_phase1`, `test:market-sign-request-phase1`, `test:market-sign-artifact-routes`, `test:agent-control-mcp`, `test:market-execution-safety-gate` |
| Retired market artifact-validation compatibility | The former external-artifact validation routes return `409 wallet_airlock_required`; Matterhorn does not accept externally signed artifacts from agent surfaces | Former validation MCP names are hidden, local fail-closed stubs and make no server request | Legacy validation commands are unsupported and are not advertised in the execution guide | `market.artifact_validation_phase2`, `test:market-artifact-validation-phase2`, `test:market-sign-artifact-routes`, `test:agent-control-mcp`, `test:market-execution-safety-gate` |
| Market execution chain gate | `GET /api/crypto/market-execution-chain` returns `matterhorn.market.execution-chain-guide.v1` with five stages: agent draft, policy and simulation, connected-wallet review, wallet authorization, and receipt reconciliation | `matterhorn_market_execution_chain` returns the same zero-authority guide; venue MCP tools can prepare handoffs and verify public receipts but cannot approve or submit | `matterhorn-work crypto execution-chain` explains the boundary without offering commands that accept signed material; the browser wallet owns approval and submission | `market.execution_chain_gate`, `test:market-execution-chain-gate`, `test:agent-control-mcp`, `test:crypto-cli-fallback`, `test:customer-readiness-ui`, `test:market-receipt-qa`, `test:market-execution-safety-gate` |
