...
| Market artifact reconciliation | No server route; offline customer evidence checker over one or more `matterhorn.market.artifact-validation.v1` outputs that verifies accepted public metadata, public audit receipt candidates, and no submit/sign capability | No MCP wrapper yet; agents should run the CLI/raw helper before attaching Phase 2 artifact-validation evidence to a customer packet | `matterhorn-work crypto artifact-reconcile --hyperliquid-artifact-validation <path> --polymarket-artifact-validation <path>` writes `matterhorn.market.artifact-reconciliation.v1`; raw helper remains `node scripts/market-artifact-reconciliation.mjs`; `matterhorn-work crypto evidence-bundle --artifact-reconciliation ... --require-artifact-reconciliation` attaches it to final market evidence | `market.artifact_reconciliation`, `test:market-artifact-reconciliation`, `test:crypto-cli-fallback`, `test:market-artifact-validation-phase2`, `test:market-customer-evidence-bundle`, `test:market-execution-safety-gate`, `test:customer-ready-crypto-smoke` |
