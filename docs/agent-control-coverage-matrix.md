...
| Unified crypto chat router | `POST /api/crypto/chat/execute` | `matterhorn_crypto_chat`; venue MCP tools remain available after routing: `matterhorn_bittensor_chat`, `matterhorn_hyperliquid_chat`, `matterhorn_polymarket_chat` | Venue CLI fallbacks: `matterhorn-work bittensor chat`, `matterhorn-work hyperliquid chat`, `matterhorn-work polymarket chat` | `test:unified-crypto-chat`, `test:agent-crypto-operator-loop`, `test:market-execution-safety-gate` |
