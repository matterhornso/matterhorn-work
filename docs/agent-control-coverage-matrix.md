...
| Market execution-readiness gate | `GET /api/crypto/market-execution-readiness` returns `matterhorn.market.execution-readiness.v1` for Hyperliquid/Polymarket and keeps live submission disabled; no submit/sign route | `matterhorn_market_execution_readiness` reads the same no-submit readiness contract for agent operators | `matterhorn-work crypto execution-readiness`; security docs live in `docs/market-execution-readiness-security-gate.md` | `test:market-execution-readiness-api`, `test:market-execution-readiness-gate`, `test:market-execution-safety-gate`, `test:customer-ready-crypto-smoke`, `test:agent-control-mcp`, `test:crypto-cli-fallback` |
