...
| `matterhorn_workflows_prompt_pack` | `GET /api/workflows/prompts` |
| `matterhorn_workflows_customer_templates` | `GET /api/workflows/templates` |
...
| `GET /api/workflows/prompts` | Returns `matterhorn.workflow.prompt-pack.v1`, the prompt-pack view of the same registry for agents that need copy-pasteable staged workflow prompts. Optional `workflow`, `category`, and `status` filters are supported. This route is read-only, rejects credential-shaped query fields, and never executes a provider action. |
| `GET /api/workflows/templates` | Returns `matterhorn.customer.workflow.template.v1`, the customer-facing template registry for Bittensor, Hyperliquid, Polymarket, wellness creator workflows, decentralized services, and blank chat. Optional `customerTemplate`, `category`, and `status` filters are supported. This route is read-only, rejects credential-shaped query fields, and is intended for app entrypoints and agent launchers. |
