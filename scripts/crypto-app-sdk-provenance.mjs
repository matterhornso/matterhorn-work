...
const PACKAGE_NAME = "@matterhorn-work/crypto-app-sdk";
const REGISTRY = "https://registry.npmjs.org/";
const REPOSITORY = "https://github.com/matterhornso/matterhorn-work";
const WORKFLOW = ".github/workflows/publish-crypto-app-sdk.yml";
...
const REPORT_VERSION = "matterhorn.crypto-app-sdk-provenance.v1";
...
function parseArguments(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (argument === "--version" || argument === "--expected-commit") {
...
function helpText() {
  return `Matterhorn Crypto App SDK published-provenance verifier

Usage:
  pnpm verify:crypto-app-sdk-provenance -- \\
    --version <exact-semver> \\
    --expected-commit <40-character-lowercase-sha> \\
    [--json]

The verifier installs one exact public-registry package with lifecycle scripts
disabled, checks npm registry signatures and transparency-backed attestations,
and binds the artifact to Matterhorn's fixed repository and release workflow.
It is read-only with respect to the registry and never publishes a package.
`;
}
...
function secureOpenFlags(exclusive = false) {
  const noFollow = constants.O_NOFOLLOW ?? 0;
  return constants.O_WRONLY | constants.O_CREAT | (exclusive ? constants.O_EXCL : 0) | noFollow;
}

function writePrivateFile(path, contents, exclusive = false) {
  mkdirSync(dirname(path), { recursive: true, mode: 0o700 });
  let descriptor;
  try {
    descriptor = openSync(path, secureOpenFlags(exclusive), 0o600);
...
function canonicalTarball(version) {
  return `${REGISTRY}${PACKAGE_NAME}/-/crypto-app-sdk-${version}.tgz`;
}
...
export function verifyCryptoAppSdkProvenance({
  version,
  expectedCommit,
  npmVersion,
  packageJson,
  ...
  const npmMinor = Number.parseInt(String(npmVersion ?? "").split(".")[1] ?? "", 10);
  ...
  if (
    ...
    packageJson?.repository?.directory !== "packages/crypto-app-sdk"
  ) {
  ...
  if (
    ...
    locked?.resolved !== canonicalTarball(version)
  ) {
  ...
  if (
    ...
    !workflow.ref.startsWith("refs/") ||
    dependencyMatches.length !== 1 ||
    run?.builder?.id !== BUILDER ||
    !invocationPattern.test(run?.metadata?.invocationId ?? "")
  ) {
...
function collectRegistryEvidence(version) {
  const temporary = mkdtempSync(join(tmpdir(), "matterhorn-sdk-provenance-"));
...
function main() {
  try {
    ...
    const report = verifyCryptoAppSdkProvenance({
