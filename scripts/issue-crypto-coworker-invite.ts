...
export type CryptoCoworkerInviteArguments = {
  serverUrl?: string;
  appUrl?: string;
  ttlMinutes: number;
  json: boolean;
...
export type CryptoCoworkerInviteResult = {
...
export function parseCryptoCoworkerInviteArguments(argv: string[]): CryptoCoworkerInviteArguments {
  const parsed: CryptoCoworkerInviteArguments = {
...
export function buildCryptoCoworkerInviteUrl(appOrigin: string, token: string): string {
  if (!INVITE_TOKEN_PATTERN.test(token)) throw new Error("Coworker invite response was invalid");
  const url = new URL("/coworker-access", canonicalOrigin(appOrigin, "App URL"));
  url.hash = new URLSearchParams({ invite: token }).toString();
  return url.toString();
}

export async function issueCryptoCoworkerInvite(input: {
  serverOrigin: string;
  appOrigin: string;
  hostToken: string;
  ttlMinutes: number;
  fetch?: FetchLike;
  timeoutMs?: number;
}): Promise<CryptoCoworkerInviteResult> {
  ...
  try {
    response = await (input.fetch ?? fetch)(`${serverOrigin}/operator/coworker-access/invites`, {
      method: "POST",
      redirect: "error",
      headers: {
    ...
    throw new Error("Matterhorn could not issue the coworker invite");
  } finally {
    clearTimeout(timer);
  }
  const text = await response.text();
  if (text.length > MAX_RESPONSE_BYTES) throw new Error("Coworker invite response was too large");
  let payload: unknown;
  try {
    payload = JSON.parse(text);
  } catch {
    throw new Error("Coworker invite response was invalid");
  }
  if (!response.ok) {
    const code = payload && typeof payload === "object" && "code" in payload
      ? String((payload as { code?: unknown }).code ?? "")
      : "";
    throw new Error(code === "coworker_invite_mode_required"
      ? "Invite-only Crypto Coworkers are not enabled"
      : "Matterhorn could not issue the coworker invite");
  }
  const invite = payload && typeof payload === "object" && "invite" in payload
    ? (payload as { invite?: unknown }).invite
    : null;
  if (!invite || typeof invite !== "object") throw new Error("Coworker invite response was invalid");
  ...
  return {
    version: "matterhorn.crypto-coworker-invite-link.v1",
    inviteUrl: buildCryptoCoworkerInviteUrl(appOrigin, token),
    expiresAt,
    safety: {
      oneTime: true,
      tokenInFragmentOnly: true,
...
function usage(): string {
  return `Issue one invite-only Matterhorn Crypto Coworker link.
...
one-time coworker token is placed in the URL fragment, which browsers do not
send in HTTP requests. Share the link with one intended tester only. The link
grants access to coworkers, not wallet signing or transaction submission.
`;
}

async function main(): Promise<void> {
  try {
    const args = parseCryptoCoworkerInviteArguments(process.argv.slice(2));
    if (args.help) {
      console.log(usage());
      return;
    }
    ...
    if (args.json) console.log(JSON.stringify(result));
      ...
      console.log("Coworker invite created. Share this once:");
      console.log(result.inviteUrl);
      console.log(`Expires: ${result.expiresAt}`);
    }
  } catch (error) {
    console.error(error instanceof Error ? error.message : "Coworker invite failed");
    process.exitCode = 1;
  }
}
