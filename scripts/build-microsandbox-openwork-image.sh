...
IMAGE_REF="${1:-matterhorn-work-microsandbox:dev}"
...
printf 'Building Matterhorn Desks micro-sandbox image %s\n' "$IMAGE_REF"
printf '  matterhorn-work@%s\n' "$OPENWORK_ORCHESTRATOR_VERSION"
printf '  matterhorn-work-server@%s\n' "$OPENWORK_SERVER_VERSION"
printf '  engine@%s\n' "$OPENCODE_VERSION"
...
printf 'Run example:\n'
printf '  docker run --rm -p 8787:8787 -e MATTERHORN_WORK_TOKEN=... -e MATTERHORN_WORK_HOST_TOKEN=... %s\n' "$IMAGE_REF"
printf 'Verify:\n'
printf '  curl http://127.0.0.1:8787/health\n'
printf '  curl -H "Authorization: Bearer $MATTERHORN_WORK_TOKEN" http://127.0.0.1:8787/workspaces\n'
