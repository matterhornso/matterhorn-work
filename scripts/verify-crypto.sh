...
echo ""
echo "========================================"
echo "  Matterhorn Work — Crypto Verification"
echo "========================================"
echo ""
...
if test -f "$MCP_PATH"; then
  ...
  check "  MCP server starts"           "echo '{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"initialize\",\"params\":{\"protocolVersion\":\"0.1.0\",\"capabilities\":{},\"clientInfo\":{\"name\":\"test\",\"version\":\"1.0\"}}}' | timeout 3 node '$MCP_PATH' 2>&1 | python3 -c \"import sys,json;d=json.loads(sys.stdin.read());sys.exit(0 if 'result' in d else 1)\""
