...
- **Bittensor** supports public SS58 wallet reads, subnet discovery, validator comparison, watches, receipt import, and coldkey/hotkey explanations. TAO transfers can move from an agent-prepared draft into a separate Finney transfer ticket, where an installed Bittensor-compatible extension reviews, signs, and broadcasts the exact call. Staking, unstaking, delegation, and advanced calls remain unsigned external-signer previews.
...
- **Polymarket** supports market discovery, outcome probability context, liquidity/orderbook reads, compliance state, watches, preview handoffs, and receipt evidence. A complete, compliance-allowed EOA BUY draft can continue in a separate Polygon wallet ticket for exact review and wallet-authorized submission. Sell orders, proxy accounts, watch-triggered orders, and unattended execution are unsupported. Compliance-blocked previews must not show executable price, size, or share fields.
...
- Every serious action uses agent draft -> separate exact review -> connected-wallet or external-signer approval -> public receipt. Matterhorn never hides signing or signs on behalf of users. The current reviewed wallet paths are Hyperliquid orders, compliance-allowed Polymarket EOA BUY orders, and Bittensor TAO transfers; other Bittensor write paths remain external-signer previews.
